...
export default function Breadcrumbs({ company }) {
  ...
  return (
    <nav className="flex min-w-0 items-center gap-2 text-sm" aria-label="Breadcrumb">
      ...
      {company?.name && (
        <>
          <span style={{ color: 'color-mix(in srgb, var(--tenant-text-muted) 52%, transparent)' }}>
            /
          </span>
          <span
            className="hidden max-w-[260px] truncate text-xs font-semibold tracking-[0.01em] xl:block"
