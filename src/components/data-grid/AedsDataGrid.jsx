...
function alignment(column) {
  return (
    column.align || (['currency', 'number', 'percent'].includes(column.type) ? 'right' : 'left')
  )
}
...
function statusTone(value) {
  ...
  if (
    /posted|paid|settled|received|clean|confirmed|arrival|departure|in_house|in house/.test(text)
  ) {
    return 'success'
  }
...
function isTotalColumn(column) {
  return column.aggregation === 'sum' || ['currency', 'number'].includes(column.type)
}
  ...
  return columns.reduce((totals, column) => {
    if (isTotalColumn(column)) {
      totals[column.accessorKey] = rows.reduce(
        (sum, row) => sum + Number(row[column.accessorKey] || 0),
        0,
      )
    }
...
function filterRows(rows, columns, search) {
  ...
  return rows.filter((row) =>
    columns.some((column) => searchableValue(row, column).includes(query)),
  )
}
...
export default function AedsDataGrid({
  ...
  const [columnOrder, setColumnOrder] = useState(columns.map((column) => columnKey(column)))
  const [columnVisibility, setColumnVisibility] = useState({})

  useEffect(() => {
    setColumnOrder((current) => {
      const incoming = columns.map((column) => columnKey(column))
  ...
  const visibleColumns = useMemo(() => {
    ...
    return columnOrder
      ...
      .filter(Boolean)
      .concat(columns.filter((column) => !columnOrder.includes(column.accessorKey)))
      .filter((column) => columnVisibility[columnKey(column)] !== false)
  }, [columns, columnOrder, columnVisibility])

  const processedRows = useMemo(
    () => sortRows(filterRows(data, columns, search), sort),
  ...
  const pagedRows = useMemo(
    () => processedRows.slice(page * pageSize, page * pageSize + pageSize),
  ...
  const { tableRef, onKeyDown } = useGridKeyboardNavigation({
    ...
    rows: Math.max(pagedRows.length, 1),
    enabled: !loading,
  })
  ...
  const resizeColumn = (key, delta) => {
    setColumnWidths((current) => ({
      ...current,
      [key]: Math.max(100, Number(current[key] || 170) + delta),
    }))
  }
  ...
  const renderCell = (row, column, rowIndex) => {
    ...
    if (column.type === 'status') {
      return (
        <span className={`aeds-grid-status ${statusTone(value)}`}>
  ...
  const renderRow = (row, rowIndex) => {
    ...
    return (
      <TableRow
        key={rowId}
        className={`aeds-grid-row ${clickable ? 'aeds-grid-row-clickable' : ''}`}
      ...
      >
        {visibleColumns.map((column, columnIndex) => {
          const frozen = frozenFirstColumn && columnIndex === 0

          return (
            <TableCell
              key={column.accessorKey}
              className={`aeds-grid-cell aeds-grid-align-${alignment(column)} ${
              ...
              style={{
                width: columnWidths[column.accessorKey] || column.width || 170,
              }}
            >
  ...
  return (
    <section className="aeds-grid-card rounded-3xl border border-slate-200 bg-white shadow-[0_18px_45px_rgba(15,23,42,0.05)]">
      <header className="aeds-grid-header border-b border-slate-200 px-5 py-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            {subtitle} · {processedRows.length.toLocaleString('en-BD')} row(s)
          </p>
        </div>
      </header>
      ...
      {error && <div className="aeds-grid-message aeds-grid-message-error">{error}</div>}

      <div className="aeds-grid-wrap overflow-x-auto">
        <Table
          ref={tableRef}
          className="aeds-grid-table min-w-full text-sm"
          role="grid"
          aria-rowcount={Math.max(pagedRows.length, 1)}
          aria-colcount={Math.max(visibleColumns.length, 1)}
          onKeyDown={onKeyDown}
        >
          <TableHeader>
            <TableRow role="row">
              {visibleColumns.map((column, columnIndex) => {
                ...
                return (
                  <TableHead
                    ...
                    className={`aeds-grid-align-${alignment(column)} ${
                      frozen ? 'aeds-grid-frozen' : ''
                    }`}
                    role="columnheader"
                    style={{
                      width: columnWidths[columnKey(column)] || column.width || 170,
                    }}
                  >
                    <div className="aeds-grid-th-inner">
                      ...
                      <button
                        ...
                        onClick={() => moveColumn(columnKey(column), 'left')}
                      ...
                      <button
                        type="button"
                        className="aeds-grid-mini"
                        onClick={() => moveColumn(columnKey(column), 'right')}
                      ...
                      <span
                        ...
                        role="separator"
                        onClick={() => resizeColumn(columnKey(column), 32)}
                        onDoubleClick={() =>
                          setColumnWidths((current) => ({
                            ...current,
            ...
            {loading ? (
              <TableRow role="row">
                <TableCell
                  ...
                  role="gridcell"
                >
                  <LoadingState variant="table" label="Loading records" rows={5} />
                </TableCell>
              </TableRow>
            ) : grouped ? (
              Object.entries(grouped).map(([groupValue, groupedRows]) => {
                const groupTotals = calculateTotals(visibleColumns, groupedRows)

                return (
                  <Fragment key={`group-${groupValue}`}>
                    <TableRow className="aeds-grid-group-row" role="row">
                      <TableCell colSpan={visibleColumns.length} role="gridcell">
                        {columns.find((column) => column.accessorKey === groupBy)?.header}:{' '}
                        <strong>{groupValue}</strong> · {groupedRows.length} row(s)
                      </TableCell>
                    </TableRow>
                    ...
                    <TableRow className="aeds-grid-subtotal">
                      {visibleColumns.map((column, index) => (
                        ...
                        >
                          {index === 0
                            ? 'Sub Total'
                            : groupTotals[column.accessorKey] !== undefined
                              ? formatValue(groupTotals[column.accessorKey], column)
                              : ''}
                        </TableCell>
                      ))}
                    </TableRow>
              ...
              <TableRow className="aeds-grid-total">
                {visibleColumns.map((column, index) => (
                  ...
                  >
                    {index === 0
                      ? 'Grand Total'
                      : totals[column.accessorKey] !== undefined
                        ? formatValue(totals[column.accessorKey], column)
                        : ''}
                  </TableCell>
                ))}
              </TableRow>
