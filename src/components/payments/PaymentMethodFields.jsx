...
export default function PaymentMethodFields({ value, onChange, errors = {}, disabled = false }) {
  ...
  const showBankAccountDiagnostic =
    needsCoaBank && !loading && bankAccounts.length > 0 && bankAccounts.length < 3

  const loadDistinctBankNames = async () => {
    const pageSize = 1000
    let from = 0
    const names = new Set()
    ...
    while (true) {
      ...
      for (const row of rows) {
        if (row?.bank_name) names.add(row.bank_name)
      }
      ...
      if (needsCoaBank) {
        ...
        jobs.push(
          query.then(({ data, error }) => {
            if (active) setBankAccounts(error ? [] : data || [])
          }),
        )
      }
      if (method === 'CARD') {
        let query = supabase
          ...
          .select(
            'id,terminal_name,merchant_id,terminal_code,bank_name,chart_of_accounts:coa_account_id(id,code,name)',
          )
          .eq('is_active', true)
          .order('terminal_name')
        if (tenantId) query = query.eq('tenant_id', tenantId)
        jobs.push(
          query.then(({ data, error }) => {
            if (active) setTerminals(error ? [] : data || [])
          }),
        )
      }
      if (method === 'CHEQUE') {
        jobs.push(
          loadDistinctBankNames().then((data) => {
  ...
  useEffect(() => {
    if (method !== 'CHEQUE' || !value?.payer_bank_name) {
      setBranches([])
      return
    }
    let active = true
    supabase
      ...
      .order('branch_name')
      .then(({ data, error }) => {
        if (active) setBranches(error ? [] : data || [])
      })
    return () => {
      active = false
    }
  ...
  return (
    <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
      {needsCoaBank && (
        <label className="block">
          <span className="mb-1 block text-xs font-semibold uppercase tracking-wide text-pine/70">
            {method === 'CHEQUE'
              ? 'Deposit To GL (Tenant Bank COA)'
              : 'Receiving GL (Tenant Bank COA)'}
          </span>
          <select
          ...
          {method === 'CHEQUE' && (
            <p className="mt-1 text-[11px] text-pine/65">
              This is your tenant deposit GL account, not the cheque issuer bank list.
            </p>
          )}
      ...
      {showBankAccountDiagnostic && (
        <div className="md:col-span-2 rounded-xl border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-800">
          <div className="font-semibold">Diagnostic: Limited tenant bank GL accounts</div>
          <div>
            Only {bankAccounts.length} active bank GL account(s) found for this tenant. If needed,
            mark additional COA rows as bank accounts.
          </div>
        </div>
      )}
      ...
      {method === 'CARD' && (
        <>
          <label className="block">
            ...
            <select
              className="input w-full"
              value={value?.pos_terminal_id || ''}
              onChange={(event) =>
                onChange({ ...value, pos_terminal_id: event.target.value, bank_account_id: '' })
              }
              disabled={disabled || loading}
              required
            >
          ...
          <label className="block">
            ...
            <select
              ...
              onChange={(event) => onChange({ ...value, card_type: event.target.value })}
              disabled={disabled}
              required
            >
          ...
          {selectedTerminal && (
            <div className="md:col-span-2 rounded-xl border border-leaf bg-mist/50 px-3 py-2 text-xs text-pine">
              ...
              <div>
                {selectedTerminal.bank_name || 'Bank'} ·{' '}
                {selectedTerminal.chart_of_accounts?.code || '—'} —{' '}
                {selectedTerminal.chart_of_accounts?.name || 'Unmapped'}
              </div>
              {(selectedTerminal.merchant_id || selectedTerminal.terminal_code) && (
                <div className="text-pine/60">
                  Merchant: {selectedTerminal.merchant_id || '—'} · Terminal:{' '}
      ...
      {method === 'CHEQUE' && (
        <>
          <label className="block">
            <span className="mb-1 block text-xs font-semibold uppercase tracking-wide text-pine/70">
              Cheque Issuer Bank (Directory)
            </span>
            <select
              className="input w-full"
              value={value?.payer_bank_name || ''}
              onChange={(event) =>
                onChange({
                  ...value,
          ...
          <label className="block">
            ...
            <select
              className="input w-full"
              value={value?.payer_routing_number || ''}
              onChange={(event) => {
                const branch = branches.find(
                  (item) => String(item.routing_number) === event.target.value,
                )
                onChange({
                  ...value,
                  payer_branch_name: branch?.branch_name || '',
                  payer_routing_number: event.target.value,
                })
              }}
              disabled={disabled || !value?.payer_bank_name}
              required
            >
              <option value="">Select branch</option>
              {branches.map((branch) => (
                <option key={branch.routing_number} value={branch.routing_number}>
                  {branch.branch_name} — {branch.district_name}
                </option>
              ))}
            </select>
          ...
          <label className="block">
            ...
            <input
              ...
              onChange={(event) => onChange({ ...value, cheque_number: event.target.value })}
              disabled={disabled}
              required
            />
          ...
          <label className="block">
            ...
            <input
              ...
              onChange={(event) => onChange({ ...value, cheque_date: event.target.value })}
              disabled={disabled}
              required
            />
