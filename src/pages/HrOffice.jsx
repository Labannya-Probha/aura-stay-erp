...
export function HrNomineePage({ userName, role, isAdmin, company }) {
  return (
    <HrPage
      title="Nominee Declaration Entry"
      subtitle="Record employee nominee / beneficiary declarations."
    >
...
export function HrCompLeavePage({ userName, role, isAdmin, company }) {
  ...
  return (
    <HrPage
      ...
      subtitle="Track and manage compensatory leave earned."
    >
      {msg && (
        <div className="px-4 py-3 rounded-lg bg-forest/10 text-forest text-sm font-medium">
          {msg}
...
export function HrFestivalLeavePage({ userName, role, isAdmin, company }) {
  return (
    <HrPage
      title="Festival Leave Management"
      subtitle="Configure festival / public holiday leave calendar."
    >
...
export function HrPayrollConfigPage({ userName, role, isAdmin, company }) {
  return (
    <HrPage
      ...
      subtitle="Configure salary structure, allowances and deduction rules."
    >
...
export function HrPayrollRegisterPage({ userName, role, isAdmin, company }) {
  return (
    <HrPage
      ...
      subtitle="View historical payroll runs and disbursement records."
    >
...
const LETTER_TYPES = [
  ...
  { value: 'JOINING', label: 'Joining Letter', extraFields: ['joiningDate'] },
  {
    ...
    extraFields: ['joiningDate', 'probation'],
  },
  ...
  { value: 'OBJECTION', label: 'Objection Letter', extraFields: ['subject', 'description'] },
  {
    ...
    extraFields: ['incidentDate', 'subject', 'allegation', 'details', 'replyDays'],
  },
  {
    value: 'WARNING',
    label: 'Warning Letter',
    extraFields: ['incidentDate', 'subject', 'description', 'warningNo'],
  },
  {
    value: 'RELIEVING',
    label: 'Letter of Dismissal/Termination',
    extraFields: ['lastWorkingDate', 'resignationType'],
  },
  { value: 'NOC', label: 'No Objection Certificate', extraFields: ['purpose'] },
  {
    value: 'EXP_CERT',
    label: 'Experience Certificate',
    extraFields: ['lastWorkingDate', 'tenure', 'skills', 'additionalNote'],
  },
  {
    value: 'SALARY_CERT',
    label: 'Employment Certificate',
    extraFields: ['purpose', 'showBreakdown'],
  },
...
export function HrLetterPage({ type, company }) {
  ...
  return (
    <HrPage
      title={selectedType?.label || 'HR Letter'}
      subtitle="Generate and print HR correspondence."
    >
      {msg && (
        <div className="px-4 py-3 rounded-lg bg-forest/10 text-forest text-sm font-medium">
          {msg}
      ...
      <div className="card p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3 pt-1">
            ...
            {selectedType.extraFields.includes('lastWorkingDate') && (
              <div>
                ...
                <input
                  type="date"
                  className="input"
                  value={extra.lastWorkingDate || ''}
                  onChange={(e) => setE('lastWorkingDate', e.target.value)}
                />
              </div>
            )}
            {selectedType.extraFields.includes('incidentDate') && (
              <div>
                ...
                <input
                  type="date"
                  className="input"
                  value={extra.incidentDate || ''}
                  onChange={(e) => setE('incidentDate', e.target.value)}
                />
              </div>
            )}
            {selectedType.extraFields.includes('probation') && (
              <div>
                ...
                <input
                  ...
                  value={extra.probation || ''}
                  onChange={(e) => setE('probation', e.target.value)}
                />
              </div>
            )}
            {selectedType.extraFields.includes('purpose') && (
              <div>
                ...
                <input
                  className="input"
                  placeholder="e.g. bank loan, visa"
                  value={extra.purpose || ''}
                  onChange={(e) => setE('purpose', e.target.value)}
                />
              </div>
            )}
            {selectedType.extraFields.includes('tenure') && (
              <div>
                ...
                <input
                  ...
                  value={extra.tenure || ''}
                  onChange={(e) => setE('tenure', e.target.value)}
                />
              </div>
            )}
            {selectedType.extraFields.includes('replyDays') && (
              <div>
                <label className="label">Reply Within</label>
                <input
                  className="input"
                  placeholder="e.g. 48 hours"
                  value={extra.replyDays || ''}
                  onChange={(e) => setE('replyDays', e.target.value)}
                />
              </div>
            )}
            ...
            {selectedType.extraFields.includes('showBreakdown') && (
              <div className="flex items-center gap-2 pt-4">
                <input
                  type="checkbox"
                  id="showBreakdown"
                  checked={!!extra.showBreakdown}
                  onChange={(e) => setE('showBreakdown', e.target.checked)}
                />
                <label htmlFor="showBreakdown" className="text-sm text-pine cursor-pointer">
            ...
            {selectedType.extraFields.includes('allegation') && (
              <div className="col-span-2">
                <label className="label">Allegation</label>
                <input
                  className="input"
                  placeholder="e.g. violated leave policy"
                  value={extra.allegation || ''}
                  onChange={(e) => setE('allegation', e.target.value)}
                />
              </div>
            )}
            {selectedType.extraFields.includes('details') && (
              <div className="col-span-2">
                ...
                <textarea
                  ...
                  rows={2}
                  value={extra.details || ''}
                  onChange={(e) => setE('details', e.target.value)}
                />
              </div>
            )}
            {selectedType.extraFields.includes('skills') && (
              <div className="col-span-2">
                <label className="label">Skills / Qualities</label>
                <input
                  className="input"
                  placeholder="e.g. excellent communication"
                  value={extra.skills || ''}
                  onChange={(e) => setE('skills', e.target.value)}
                />
              </div>
            )}
            {selectedType.extraFields.includes('additionalNote') && (
              <div className="col-span-2">
                ...
                <textarea
                  className="input"
                  rows={2}
                  value={extra.additionalNote || ''}
                  onChange={(e) => setE('additionalNote', e.target.value)}
                />
              </div>
            )}
          </div>
        ...
        <div className="flex justify-end pt-1">
          <button className="btn-primary" onClick={openPrint}>
      ...
      {printDoc && (
        <PrintPortal
          title={`${selectedType?.label} — ${printDoc.employee.full_name}`}
          onClose={() => setPrintDoc(null)}
        >
          <HrLetterDoc
            type={printDoc.type}
            employee={printDoc.employee}
            extra={printDoc.extra}
            company={company}
            date={printDoc.date}
          />
        </PrintPortal>
...
export function HrAttendanceRegisterPage({ flash }) {
  return (
    <HrPage
      ...
      subtitle="Daily attendance marking for all active employees."
    >
...
export function HrEmployeeRegisterPage({ role }) {
  return (
    <HrPage
      title="Employee Register (Form-8)"
      subtitle="Statutory employee register as per Bangladesh Labour Rules."
    >
...
export function HrServiceBookRegPage({ userName }) {
  return (
    <HrPage
      ...
      subtitle="View and print service book records for all employees."
    >
...
export function HrIncidentsPage({ userName, flash }) {
  return (
    <HrPage
      ...
      subtitle="Log and track workplace incidents and disciplinary actions."
    >
...
export function HrCompliancePage({ role }) {
  return (
    <HrPage
      title="Compliance"
      subtitle="Bangladesh Labour Law compliance checklists and statutory forms."
    >
...
function AttendanceTab({ flash }) {
  ...
  const mark = async (empId, status) => {
    ...
    const { error } = await supabase
      ...
      .upsert(
        { employee_id: empId, att_date: date, status },
    ...
    <div className="space-y-4">
      ...
      <AedsDataGrid
        ...
        data={rows.map((row) => ({
          ...
          balance:
            (Number(row.leave_types?.annual_days) || 0) - taken(row.employee_id, row.leave_type_id),
        }))}
        columns={[
            ...
            cell: ({ row }) => (
              <button
                ...
                className={`status-chip ${
                  row.used ? 'bg-stone-200 text-stone-700' : 'bg-forest/15 text-forest'
                }`}
              >
...
const SPLIT_OF_BASIC = { house_rent: 0.5, conveyance: 0.35, medical: 0.2 }
const MONTH_NAMES = [
  'January',
  'February',
  'March',
...
function resolveSalaryStructure(structures, employeeId) {
  const employeeStructure = structures.find(
    (row) => row.employee_id === employeeId && row.is_active,
  )
  if (employeeStructure) return employeeStructure
  const defaultStructure = structures.find(
    (row) => !row.employee_id && row.is_default && row.is_active,
  )
...
function PayrollTab({ flash, userName, canApprove, isAdmin, company }) {
  ...
  const generateRun = async ({ silent = false } = {}) => {
    ...
    try {
      ...
      if (existing) {
        flash(
          `Payroll for ${MONTH_NAMES[month - 1]} ${year} already exists — open it below to review.`,
        )
        setBusy(false)
        return
      }
      ...
      const [{ data: emps }, { data: allowances }] = await Promise.all([
        ...
        supabase
          ...
          .select('*')
          .eq('is_active', true)
          .eq('allowance_name', 'Internet/Telephone Allowance'),
      ])
      const allowanceMap = Object.fromEntries(
        (allowances || []).map((a) => [a.designation, +a.amount]),
      )
      ...
      for (const e of emps || []) {
        const { count } = await supabase
          ...
          .eq('employee_id', e.id)
          .eq('status', 'A')
          .gte('att_date', periodStart)
          .lte('att_date', periodEnd)
        const absentDays = count || 0
        const gross = +e.gross_salary || 0
        const structure = resolveSalaryStructure(salaryStructures, e.id)
        const basicPct = Number(structure?.basic_pct ?? BASIC_PCT_OF_GROSS)
        const houseRentPct = Number(structure?.house_rent_pct ?? SPLIT_OF_BASIC.house_rent)
        const conveyancePct = Number(structure?.conveyance_pct ?? SPLIT_OF_BASIC.conveyance)
        const medicalPct = Number(structure?.medical_pct ?? SPLIT_OF_BASIC.medical)
        const basic = +(gross * basicPct).toFixed(2)
        const houseRent = +(basic * houseRentPct).toFixed(2)
        const conveyance = +(basic * conveyancePct).toFixed(2)
        const medical = +(basic * medicalPct).toFixed(2)
        const internetAllowance = Number(
          structure?.internet_allowance ?? allowanceMap[e.designation] ?? allowanceMap.DEFAULT ?? 0,
        )
        ...
        const netPayable = +(gross - absentDeduction + otherAllowance).toFixed(2)
        slipsToInsert.push({
      ...
      if (slipsToInsert.length === 0) {
        if (!silent) flash('No active employees to run payroll for.')
        setBusy(false)
        return
      }
      ...
      await openRun(run)
      flash(
        `Payroll generated for ${MONTH_NAMES[month - 1]} ${year} — ${slipsToInsert.length} payslip(s).`,
      )
    } catch (e) {
      flash(e.message)
    }
  ...
  const updateSlip = async (slip, field, value) => {
    ...
    const { error } = await supabase
      .from('payslips')
      .update({ [field]: value, net_payable: n.net_payable })
      .eq('id', slip.id)
    if (error) flash(error.message)
  }
  ...
  const submitForReview = async () => {
    if (
      !window.confirm(
        `Submit payroll for ${MONTH_NAMES[active.period_month - 1]} ${active.period_year} to review?`,
      )
    )
      return
    try {
      const { error } = await supabase.rpc('payroll_submit_for_review', {
        p_payroll_run_id: active.id,
        p_note: `Submitted by ${userName}`,
      })
      if (error) throw error
      flash('Payroll submitted for review.')
    } catch (e) {
      flash(e.message)
      return
    }
    await loadRuns()
    setActive((a) => ({ ...a, status: 'PENDING_REVIEW' }))
  }

  const approveRun = async () => {
    if (
      !window.confirm(
        `Approve payroll for ${MONTH_NAMES[active.period_month - 1]} ${active.period_year} and post journal?`,
      )
    )
      return
    try {
      const { data: jvId, error } = await supabase.rpc('payroll_approve_and_post_jv', {
        p_payroll_run_id: active.id,
        p_note: `Approved by ${userName}`,
      })
      if (error) throw error
  ...
  const lockRun = async () => {
    if (!window.confirm('Lock this payroll run? Locked runs are immutable.')) return
    const { error } = await supabase.rpc('payroll_lock_run', {
      p_payroll_run_id: active.id,
      p_note: `Locked by ${userName}`,
    })
    if (error) {
      flash(error.message)
      return
    ...
    await loadRuns()
    setActive((a) => ({ ...a, status: 'LOCKED' }))
    flash('Payroll locked.')
  }
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-3">
            <button className="btn-ghost !py-1" onClick={() => setActive(null)}>
              ← All runs
            </button>
            <h3 className="font-display font-semibold text-pine flex items-center gap-2">
              <Wallet size={16} className="text-forest" /> {MONTH_NAMES[active.period_month - 1]}{' '}
              {active.period_year}
            </h3>
            <span className={`status-chip ${statusChip}`}>{active.status}</span>
          </div>
          <div className="flex gap-2">
            {canApprove && active.status === 'DRAFT' && (
              <button className="btn-primary !py-1.5" onClick={submitForReview}>
                <Check size={14} /> Submit Review
              </button>
            )}
            {canApprove && active.status === 'PENDING_REVIEW' && (
              <button className="btn-primary !py-1.5" onClick={approveRun}>
                <Check size={14} /> Approve
              </button>
            )}
            {isAdmin && active.status === 'APPROVED' && (
              <button className="btn-amber !py-1.5" onClick={lockRun}>
                <Wallet size={14} /> Lock Run
              </button>
            )}
          </div>
        </div>
        {locked && (
          <div className="px-4 py-2 rounded-lg bg-amber/10 text-amber text-sm">
            This run is {active.status.toLowerCase()} and follows gated transitions — amounts are
            locked.
          </div>
        )}
        <AedsDataGrid
          ...
          columns={[
            ...
            {
              accessorKey: 'internet_allowance',
              header: 'Internet',
              type: 'currency',
              aggregation: 'sum',
              width: 140,
              ...
              cell: ({ row }) =>
                locked ? (
                  ...
                  <input
                    ...
                    onClick={(event) => event.stopPropagation()}
                    onBlur={(event) =>
                      updateSlip(row, 'advance_deduction', +event.target.value || 0)
                    }
                  />
                ),
            },
        ...
        {printSlip && (
          <PrintPortal
            title={`Payslip — ${printSlip.full_name}`}
    ...
    <div className="space-y-4">
      <div className="card p-4 flex items-end gap-3 flex-wrap">
        ...
        <div>
          ...
          <input
            type="number"
            className="input !w-28 money"
            value={year}
            onChange={(e) => setYear(+e.target.value)}
          />
        </div>
        {canApprove && (
          <button className="btn-primary" disabled={busy} onClick={generateRun}>
            <Wallet size={15} /> {busy ? 'Generating…' : 'Generate payroll'}
          </button>
        )}
      </div>
      <AedsDataGrid
        ...
        columns={[
          ...
          { accessorKey: 'status_label', header: 'Status', type: 'status', width: 140 },
    ...
    <div style={{ maxWidth: 600, margin: '0 auto', color: '#000' }}>
      ...
      <table style={{ width: '100%', fontSize: 11, marginBottom: 10 }}>
        <tbody>
          <tr>
            <td>
              <b>Employee:</b> {slip.full_name}
            </td>
            <td style={{ textAlign: 'right' }}>
              <b>Code:</b> {slip.emp_code}
            </td>
          </tr>
          <tr>
            <td>
              <b>Designation:</b> {slip.designation || '—'}
            </td>
            <td style={{ textAlign: 'right' }}>
              <b>Department:</b> {slip.department || '—'}
            </td>
          </tr>
        </tbody>
      </table>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ background: '#eee' }}>
            <th style={cell}>Salary Breakdown</th>
            <th style={{ ...cell, textAlign: 'right' }}>Amount</th>
            <th style={cell}>Deductions</th>
            <th style={{ ...cell, textAlign: 'right' }}>Amount</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            ...
            <td style={rt}>{fmtBDT(slip.basic)}</td>
            <td style={cell}>Absent ({slip.absent_days} day(s))</td>
            <td style={rt}>{fmtBDT(slip.absent_deduction)}</td>
          </tr>
          <tr>
            <td style={cell}>House Rent</td>
            <td style={rt}>{fmtBDT(slip.house_rent)}</td>
            <td style={cell}>Advance</td>
            <td style={rt}>{fmtBDT(slip.advance_deduction)}</td>
          </tr>
          <tr>
            <td style={cell}>Transportation</td>
            <td style={rt}>{fmtBDT(slip.conveyance)}</td>
            <td style={cell}>Other</td>
            <td style={rt}>{fmtBDT(slip.other_deduction)}</td>
          </tr>
          <tr>
            <td style={cell}>Medical Allowance</td>
            <td style={rt}>{fmtBDT(slip.medical)}</td>
            <td style={cell}></td>
            <td style={rt}></td>
          </tr>
          <tr>
            <td style={cell}>Internet/Telephone</td>
            <td style={rt}>{fmtBDT(slip.internet_allowance)}</td>
            <td style={cell}></td>
            <td style={rt}></td>
          </tr>
          {+slip.other_allowance > 0 && (
            <tr>
              <td style={cell}>Other allowance</td>
              <td style={rt}>{fmtBDT(slip.other_allowance)}</td>
              <td style={cell}></td>
              <td style={rt}></td>
            </tr>
          )}
        ...
        <tfoot>
          <tr style={{ fontWeight: 700, background: '#f5f5f5' }}>
            ...
            <td style={rt}>{fmtBDT(slip.gross_salary)}</td>
            <td style={cell}>Total deduction</td>
            <td style={rt}>
              {fmtBDT(+slip.absent_deduction + +slip.advance_deduction + +slip.other_deduction)}
            </td>
          </tr>
        </tfoot>
      </table>
      ...
      >
        ...
        <span>{fmtBDT(slip.net_payable)}</span>
      </div>
          ...
          { accessorKey: 'reported_by', header: 'Reported By', width: 170 },
          {
            ...
            cell: ({ row }) => (
              <button
                ...
                className={`status-chip ${
                  row.status === 'OPEN' ? 'bg-amber/20 text-amber' : 'bg-forest/15 text-forest'
                }`}
              >
                {row.status}
              </button>
