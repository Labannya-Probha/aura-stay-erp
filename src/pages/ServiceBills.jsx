  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        side="top"
        align="center"
        className="w-auto max-w-[220px] px-2.5 py-1.5 text-xs text-pine"
      >
        {message}
      </PopoverContent>
    </Popover>
...
export default function Facilities({ userName, isAdmin }) {
  ...
  return (
    <div>
      ...
      {tab === 'Items' && (
        <ItemsManager items={items} reload={load} isAdmin={isAdmin} flash={flash} />
      )}
...
function NewSale({ items, taxConfig, userName, flash, onDone }) {
  ...
  const rate = rateFor(taxConfig, 'OTHER', todayISO()) ||
    rateFor(taxConfig, 'FOOD', todayISO()) || { vat_pct: 0, service_charge_pct: 0 }
  ...
  const addItem = (fi) =>
    setCart((prev) => {
      ...
      return [
        ...prev,
        {
          facility_item_id: fi.id,
          item_name: `${fi.name} (${fi.unit})`,
          unit: fi.unit,
          qty: 1,
          unit_price: Number(fi.default_price),
        },
      ]
    })
  const addCustom = () =>
    setCart((prev) => [
      ...prev,
      { facility_item_id: null, item_name: 'Custom item', unit: 'pc', qty: 1, unit_price: 0 },
    ])
  const bump = (i, d) =>
    setCart((prev) =>
      prev
        .map((c, x) => (x === i ? { ...c, qty: Math.max(0, c.qty + d) } : c))
        .filter((c) => c.qty > 0),
    )
  const setPrice = (i, v) =>
    setCart((prev) => prev.map((c, x) => (x === i ? { ...c, unit_price: Number(v) || 0 } : c)))
  const setName = (i, v) =>
    setCart((prev) => prev.map((c, x) => (x === i ? { ...c, item_name: v } : c)))
  const removeLine = (i) => setCart((prev) => prev.filter((_, x) => x !== i))

  const persist = async (statusFields) => {
    const payload = {
      outlet: catMeta.outlet,
      order_type: 'TAKEAWAY',
      table_no: null,
  ...
  const snapshot = () => {
    ...
    const out = lines.map((l) => {
      ...
      const o = {
        ...
        charge_type: cat,
        description: `${l.item_name} × ${l.qty}`,
        base_amount: l.line_total,
        status: 'PAID',
      }
      keys.forEach((k) => {
        o[k] = +(t[k] * ratio).toFixed(2)
      })
      return o
    })
    if (out.length)
    ...
    out.forEach((o) => {
      o.total = +(o.base_amount - o.discount + o.service_charge + o.vat).toFixed(2)
    })
    return out
  }
  ...
  const payNow = async () => {
    if (cart.length === 0) {
      flash('Add at least one item.', 'warning')
      return
    }
    const validPayments = payments.filter((p) => Number(p.amount) > 0)
    if (validPayments.length === 0) {
      flash('Enter at least one payment amount.', 'warning')
      return
    }
    const totalPaid = validPayments.reduce((a, p) => a + Number(p.amount), 0)
    if (totalPaid < t.total - 0.01) {
      flash(`Amount short by ${fmtBDT(t.total - totalPaid)} — add more payment.`, 'warning')
      return
    }
    const primaryMethod = [...validPayments].sort((a, b) => Number(b.amount) - Number(a.amount))[0]
      .method
    ...
    try {
      ...
      if (order.reservation_id) {
        ...
        await supabase.from('payments').insert(
          withTenantInsert({
            ...
            notes: catMeta.outlet,
          }),
        )
        await withTenant(supabase.from('pos_orders').update({ folio_charge_id: fc.id })).eq(
          'id',
          order.id,
        )
      } else if (issueMushak) {
        const { data: inv, error: ve } = await supabase
          .from('invoices')
          .insert(
            withTenantInsert({
              invoice_type: 'MUSHAK_63',
              pos_order_id: order.id,
              buyer_name: order.guest_name || 'Walk-in Customer',
              buyer_address: '',
              buyer_bin: '',
              totals: {
                base: t.base_amount,
                discount: t.discount,
                taxable_value: +(t.base_amount - t.discount).toFixed(2),
                service_charge: t.service_charge,
                vat: t.vat,
                grand_total: t.total,
                paid: t.total,
        ...
        await withTenant(supabase.from('pos_orders').update({ invoice_id: inv.id })).eq(
          'id',
          order.id,
        )
      }
      ...
      onDone({
        type: 'RECEIPT',
        order: { ...order, status: 'SETTLED', payment_method: primaryMethod },
        items: oi,
        mushakNo,
      })
      flash(
        `${order.order_no} settled — ${paymentSummary}.${mushakNo ? ` Mushak-6.3 ${mushakNo} issued.` : ''}`,
        'success',
      )
    } catch (e) {
  ...
  const chargeToRoom = async () => {
    ...
    if (!link.reservation_id) {
      flash('Link an in-house guest first.', 'warning')
      return
    }
    setBusy(true)
    try {
    ...
    <div className="grid grid-cols-1 xl:grid-cols-5 gap-4">
      ...
      <div className="xl:col-span-3">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
          {visible.map((fi) => (
            ...
            >
              ...
              <div className="money text-forest text-sm mt-1">
                {fmtBDT(fi.default_price)} <span className="text-pine/40">/ {fi.unit}</span>
              </div>
            </button>
          ))}
          <button
            onClick={addCustom}
            className="card p-3 text-left border-dashed hover:border-forest text-pine/60"
          >
        ...
        {rate && (
          <p className="text-xs text-pine/50 mt-3">
            VAT {rate.vat_pct}% · SC {rate.service_charge_pct}% (Settings → Tax Policy)
          </p>
        )}
      </div>
      ...
      <div className="xl:col-span-2 space-y-3">
        <div className="card p-4">
          <h3 className="font-display font-semibold text-pine mb-2">
            Facilities &amp; Shop — current sale
          </h3>
          ...
          <div className="max-h-64 overflow-auto divide-y divide-leaf/60">
            {cart.map((c, i) => (
              <div key={i} className="py-2 space-y-1">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <button className="w-6 h-6 rounded bg-leaf text-pine" onClick={() => bump(i, -1)}>
                    <Minus size={13} className="mx-auto" />
                  </button>
                  <span className="money w-7 text-center font-semibold">{c.qty}</span>
                  <button
                    className="w-6 h-6 rounded bg-forest text-white"
                    onClick={() => bump(i, 1)}
                  >
                  ...
                  <input
                    type="number"
                    className="input !w-28 !py-1 money text-right"
                    value={c.unit_price}
                    onChange={(e) => setPrice(i, e.target.value)}
                  />
                  <span className="money flex-1 text-right text-sm font-semibold">
            ...
            {cart.length === 0 && (
              <p className="text-sm text-pine/50 py-4 text-center">
                Tap items to add — every price stays editable.
              </p>
            )}
          </div>
            ...
            {payments.map((p, i) => (
              <div key={i} className="flex gap-2 items-center flex-wrap sm:flex-nowrap">
                <SearchableSelect
                  ...
                  onChange={(value) =>
                    setPayments((prev) =>
                      prev.map((x, j) => (j === i ? { ...x, method: value } : x)),
                    )
                  }
            ...
            return (
              <div className="flex justify-between gap-2 flex-wrap text-xs font-medium px-1">
                <span className="text-pine/60">
                  Paid: <span className="money text-forest font-bold">{fmtBDT(paid)}</span>
                </span>
          ...
          {!link.reservation_id && (
            <label className="flex items-center gap-2 text-xs text-pine/70">
              <input
                type="checkbox"
                checked={issueMushak}
                onChange={(e) => setIssueMushak(e.target.checked)}
              />
...
function SalesList({ setPrintDoc, isAdmin, flash }) {
  ...
  const withItems = async (o) =>
    (await withTenant(supabase.from('pos_order_items').select('*')).eq('order_id', o.id)).data || []
  const printReceipt = async (o) =>
    setPrintDoc({ type: 'RECEIPT', order: o, items: await withItems(o), mushakNo: null })
  const printMushak = async (o) => {
    const { data: inv } = await withTenant(supabase.from('invoices').select('*'))
      .eq('id', o.invoice_id)
      .single()
    if (inv) setPrintDoc({ type: 'MUSHAK', invoice: inv, refNo: o.order_no })
  }
  const voidSale = async (o) => {
    if (o.folio_charge_id)
      await withTenant(supabase.from('folio_charges').delete()).eq('id', o.folio_charge_id)
    if (o.reservation_id)
      await withTenant(supabase.from('payments').delete())
        .eq('reservation_id', o.reservation_id)
        .eq('reference', o.order_no)
    if (o.invoice_id) await withTenant(supabase.from('invoices').delete()).eq('id', o.invoice_id)
    await withTenant(
      supabase.from('pos_orders').update({ status: 'CANCELLED', notes: '[VOIDED by admin]' }),
    ).eq('id', o.id)
    flash(`${o.order_no} voided & reversed.`, 'warning')
    load()
  }
          ...
          <tbody>
            {rows.map((o) => (
              <tr key={o.id} className="hover:bg-leaf/20">
                ...
                <td className="td">
                  <span className={`status-chip ${chip[o.status]}`}>
                  ...
                  <div className="flex justify-end gap-1.5">
                    ...
                    {o.invoice_id && (
                      <ActionPopoverButton
                        ...
                        message="Print Mushak 6.3"
                        onClick={() => printMushak(o)}
                      >
                        6.3
                      </ActionPopoverButton>
                    )}
                    {isAdmin && ['SETTLED', 'CHARGED_TO_ROOM'].includes(o.status) && (
                      <ActionPopoverButton
                        className="btn-ghost !py-1 !px-2 text-red-500 text-xs"
                        message="Void this sale and reverse postings"
                        onClick={() => voidSale(o)}
                      >
                        Void
                      </ActionPopoverButton>
            ...
            {rows.length === 0 && (
              <tr>
                <td className="td text-pine/50" colSpan={7}>
                  No facility sales yet.
                </td>
              </tr>
            )}
          </tbody>
...
function ItemsManager({ items, reload, isAdmin, flash }) {
  ...
  const updatePrice = async (it, price) => {
    ...
    const { error } = await withTenant(
      supabase.from('facility_items').update({ default_price: +price }),
    ).eq('id', it.id)
    if (error) {
      flash(error.message, 'error')
      return
  ...
  const toggle = async (it) => {
    await withTenant(supabase.from('facility_items').update({ is_active: !it.is_active })).eq(
      'id',
      it.id,
    )
      ...
      {isAdmin && (
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-2 mb-4 items-end">
          ...
          <div>
            ...
            <SearchableSelect
              value={n.unit}
              onChange={(value) => setN({ ...n, unit: value })}
              options={['pc', 'pkt', 'jar', 'kg', 'hour', 'day']}
              placeholder="Unit"
          ...
          <div>
            ...
            <input
              ...
              value={n.default_price}
              onChange={(e) => setN({ ...n, default_price: e.target.value })}
            />
          </div>
