...
export default function PaymentConfigurationPage(){
 ...
 const {terminals,settlementAccounts,search,setSearch,isLoading,isRefreshing,isSaving,pendingTerminalId,error,lastRefresh,refresh,createTerminal,updateTerminal,toggleTerminalStatus,removeTerminal}=usePaymentConfiguration(tenantId)
 ...
 return <section className="space-y-5">
  ...
  <div className="overflow-hidden rounded-2xl border bg-white shadow-sm"><PaymentTerminalToolbar search={search} onSearchChange={setSearch} onRefresh={refresh} onCreate={create} isRefreshing={isRefreshing} lastRefresh={lastRefresh}/>{isLoading?<Skeleton/>:terminals.length?<PaymentTerminalTable terminals={terminals} pendingTerminalId={pendingTerminalId} onEdit={edit} onToggleStatus={toggle} onDelete={setDeleteTarget}/>:<EmptyState onCreate={create}/>}</div>
  <PaymentTerminalDialog open={editorOpen} terminal={selectedTerminal} settlementAccounts={settlementAccounts} isSaving={isSaving} error={error} onClose={()=>{if(!isSaving){setEditorOpen(false);setSelectedTerminal(null)}}} onSubmit={save}/><ConfirmDeleteDialog open={Boolean(deleteTarget)} terminal={deleteTarget} isDeleting={pendingTerminalId===deleteTarget?.id} onCancel={()=>setDeleteTarget(null)} onConfirm={confirmDelete}/></section>
}
