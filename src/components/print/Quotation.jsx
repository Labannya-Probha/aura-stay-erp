    ...
    <div className="print-a4-doc" style={{ maxWidth: '186mm', margin: '0 auto' }}>
      ...
      <table style={{ width: '100%', fontSize: 12, marginBottom: 10 }}>
        <tbody>
          <tr>
            <td>
              <b>Quotation Ref:</b> {res.res_no}
            </td>
            <td style={{ textAlign: 'right' }}>
              <b>Date:</b> {fmtDate(todayISO())}
            </td>
          </tr>
          <tr>
            <td>
              <b>Guest:</b> {guest?.full_name || res.reservation_name || '—'}
              {guest?.phone ? ` · ${guest.phone}` : ''}
            </td>
            <td style={{ textAlign: 'right' }}>
              <b>Valid until:</b> {fmtDate(validUntil)}
            </td>
          </tr>
        </tbody>
      </table>
          ...
          {sum.sc > 0 && (
            <tr>
              <td style={cell} colSpan={6}>
                Service charge {rate.service_charge_pct}%
              </td>
              <td style={rt}>{fmtBDT(sum.sc)}</td>
            </tr>
          )}
          <tr>
            <td style={cell} colSpan={6}>
            ...
            <td style={rt}>{fmtBDT(sum.vat)}</td>
          </tr>
          <tr style={{ fontWeight: 700, background: soft }}>
            <td style={{ ...cell, borderColor: line }} colSpan={6}>
              GRAND TOTAL
            </td>
            <td style={{ ...rt, borderColor: line, color: primary }}>
              {fmtBDT(sum.total + sum.sc + sum.vat)}
            </td>
          </tr>
        </tfoot>
      </table>
      ...
      <div style={{ marginTop: 28, fontSize: 12 }}>
        For {company?.legal_name || companyName || 'the company'},<br />
        <br />
        <br />
      ...
      {company?.invoice_footer && (
        <div style={{ textAlign: 'center', fontSize: 10, marginTop: 16, color: muted }}>
          {company.invoice_footer}
        </div>
      )}
    </div>
