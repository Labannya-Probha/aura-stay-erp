...
export default function BookingCalendar({ openReservation, onNewReservation }) {
  ...
  const load = async () => {
    ...
    const [{ data: rm, error: rmErr }, { data: rr, error: rrErr }] = await Promise.all([
      supabase.from('rooms').select('*').eq('is_active', true).order('room_no'),
      supabase
        .from('reservation_rooms')
        .select(
          'id, room_id, from_date, to_date, reservations!inner(id, res_no, reservation_name, status, check_in, check_out, source, advance_paid, guests:primary_guest_id(full_name, phone))'
        )
    ...
    if (rmErr || rrErr) {
      flash(rmErr?.message || rrErr?.message || 'Failed to load booking calendar.')
      setLoading(false)
      return
    }
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button className="btn-ghost !px-2" onClick={() => setYm(shiftMonth(ym, -1))}>
            <ChevronLeft size={16} />
          </button>
      ...
      <div className="flex flex-wrap gap-3 text-xs items-center">
        <span className="font-semibold text-pine">
          Occupancy this month: <span className="money">{occPct.toFixed(1)}%</span> ({occRoomNights}/{rooms.length * days.length} room-nights)
        </span>
      ...
      <div className="card overflow-auto" style={{ WebkitOverflowScrolling: 'touch' }}>
        {loading ? (
          ...
          <table className="border-collapse" style={{ minWidth: 'max-content' }}>
            <thead>
              <tr>
                <th className="th sticky left-0 bg-white z-10 border-r border-leaf" style={{ minWidth: 150 }}>
                  Room
                </th>
                {days.map((d) => {
                  ...
                  return (
                    <th key={d} className={`th text-center !px-1 ${isWe ? 'bg-leaf/40' : ''}`} style={{ minWidth: DAY_W }} title={d}>
                      {d.slice(8)}
                    </th>
                  )
                })}
              ...
              {rooms.map((r) => {
                ...
                while (i < days.length) {
                  ...
                  if (res) {
                    ...
                    const advancePaid =
                      typeof res.advance_paid === 'boolean'
                        ? res.advance_paid
                        : ['CONFIRMED', 'CHECKED_IN', 'CHECKED_OUT', 'SETTLED'].includes(res.status)

                    tds.push(
                      <td key={d} colSpan={span} className="border border-leaf/60 p-0" style={{ height: 52 }}>
                        <button
                          onClick={() => openReservation(res.id)}
                          title={`${name} (${res.status})`}
                          className="w-full h-full text-left text-[10px] leading-tight px-1.5 py-1 overflow-hidden"
                        ...
                        >
                          ...
                          <div className="truncate">{resNo}</div>
                          <div className="truncate">Advance: {advancePaid ? 'Paid' : 'Unpaid'}</div>
                        </button>
                      </td>
                    )
                    i += span
                    ...
                    tds.push(
                      <td key={d} className="border border-leaf/60 p-0" style={{ minWidth: DAY_W, height: 52 }}>
                        <button
