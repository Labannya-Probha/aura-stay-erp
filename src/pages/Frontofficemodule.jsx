      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-4 gap-3">
        <div>
          <h1 className="font-display text-2xl font-bold text-pine">
            {res.reservation_name || guest?.full_name}
          </h1>
          <span className={`status-chip ${STATUS_COLORS[res.status]}`}>
        ...
        <div className="sm:text-right">
          ...
          <div
            className={`font-display text-2xl font-bold money ${due > 0 ? 'text-red-600' : 'text-forest'}`}
          >
            {fmtBDT(due)}
          </div>
        </div>
      ...
      {tab === 'Overview' && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
          <div className="card p-3">
            ...
            <div className="font-display text-xl font-bold money text-pine">
              {fmtBDT(totals.grand_total)}
            </div>
          </div>
          <div className="card p-3">
          ...
          <div className="card p-3">
            ...
            <div
              className={`font-display text-xl font-bold money ${due > 0 ? 'text-red-600' : 'text-forest'}`}
            >
              {fmtBDT(due)}
            </div>
          </div>
...
function Overview({
    ...
    for (const ad of addonList) {
      if (ad.id) {
        await supabase
          ...
          .update({ label: ad.label, price: ad.price, qty: ad.qty })
          .eq('id', ad.id)
      } else {
  ...
  const postAddonCharges = async () => {
    if (unposted.length === 0) {
      flash('No unposted addon items to post.')
      return
    }
    setPosting(true)
    try {
  ...
  const requestRoomClearance = async () => {
    ...
    try {
      ...
      flash(
        created > 0
          ? `Room clearance request sent for ${created} room(s).`
          : 'Room clearance request is already pending.',
      )
    } catch (e) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      ...
      <div className="card p-5 lg:col-span-3">
        ...
        <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm">
          <div>
            <dt className="label">Primary guest</dt>
            <dd className="font-semibold">
              {res.salutation ? `${res.salutation} ` : ''}
              {guest?.full_name || '—'}
            </dd>
          </div>
          <div>
          ...
          <div>
            <dt className="label">Guest type</dt>
            <dd>{res.guest_type || 'Individual'}</dd>
          </div>
          <div>
            <dt className="label">Reservation name</dt>
            <dd>
              {res.reservation_name || '—'}
              {res.use_reservation_name_only && (
                <span className="text-xs text-pine/50"> (used everywhere)</span>
              )}
            </dd>
          </div>
          <div>
            ...
            <dd>
              {res.discount_type === 'fixed'
                ? Number(res.discount_val) > 0
                  ? `${fmtBDT(res.discount_val)} fixed`
                  : '—'
          ...
          <div>
            <dt className="label">Rooms assigned</dt>
            <dd>
              {resRooms.length
                ? resRooms.map((r) => r.rooms?.room_no).join(', ')
                : 'Not yet assigned'}
            </dd>
          </div>
        ...
        {isCompany && (
          <>
            ...
            <dl className="grid grid-cols-1 sm:grid-cols-3 gap-x-6 gap-y-3 text-sm">
              <div>
                <dt className="label">Commission rate</dt>
                <dd className="font-semibold money">{Number(res.commission_pct) || 0}%</dd>
              </div>
              <div>
                <dt className="label">Vat/VDS</dt>
                <dd className="font-semibold money">{Number(res.vat_vds_pct) || 0}%</dd>
              </div>
              <div>
                <dt className="label">Tax/TDS</dt>
                <dd className="font-semibold money">{Number(res.tax_tds_pct) || 0}%</dd>
              </div>
            </dl>
          </>
        )}
      ...
      <div className="card p-5 lg:col-span-3">
        ...
        {!quote ? (
          <p className="text-sm text-pine/50 py-4">
            No quotation created yet. Click "+ New quotation" to create one.
          </p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {/* Col 1 — IDs & Guest */}
            <div className="space-y-1">
              <div className="text-xs text-pine/50 uppercase tracking-wide font-semibold">
                Quotation
              </div>
              <div className="font-bold text-forest money text-sm">{res.res_no}</div>
              <div className="font-semibold text-pine text-sm">
                {guest?.full_name || res.reservation_name || '—'}
              </div>
            ...
            <div className="space-y-1">
              ...
              <div className="text-xs text-pine/60">
                {quote.room_count ?? resRooms.length} room
                {(quote.room_count ?? resRooms.length) !== 1 ? 's' : ''} ·{' '}
            ...
            <div className="space-y-1">
              ...
              <div className="font-bold text-forest money text-xl">
                {fmtBDT(quote.total_amount)}
              </div>
      ...
      {quoteEditorOpen && (
        <div className="fixed inset-0 bg-ink/60 z-50 flex items-start justify-center overflow-auto p-3 sm:p-6">
          <div className="card max-w-lg w-full p-4 sm:p-6 my-3 sm:my-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <SearchableSelect
                    ...
                    options={['', 'Mr.', 'Mrs.', 'Ms.', 'Dr.', 'Prof.'].map((s) => ({
                      value: s,
                      label: s || '—',
                    }))}
                    placeholder="Select…"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditForm({ ...editForm, pax_adults: e.target.value })}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditForm({ ...editForm, pax_children: e.target.value })}
                  />
                </div>
                <div>
                ...
                <div className="col-span-1 sm:col-span-2">
                  ...
                  <input
                    ...
                    onChange={(e) => setEditForm({ ...editForm, reservation_name: e.target.value })}
                  />
                  <div className="flex items-center gap-2 mt-1">
                    <input
                      type="checkbox"
                      id="useResName"
                      checked={editForm.use_reservation_name_only}
                      onChange={(e) =>
                        setEditForm({ ...editForm, use_reservation_name_only: e.target.checked })
                      }
                    />
                    <label htmlFor="useResName" className="text-xs text-pine/60">
            ...
            <fieldset className="border border-leaf rounded-xl p-4 mb-4">
              <legend className="text-xs font-bold text-pine/60 px-2 uppercase tracking-wide">
                Rooms — Pick from dropdown, each with its own dates
              </legend>
              <div className="flex gap-2 mb-3">
                <SearchableSelect
                  ...
                  options={roomsAll
                    ...
                    .map((r) => ({
                      value: r.id,
                      label: `${r.room_no}${r.room_name ? ` - ${r.room_name}` : ''} (${r.room_type})`,
                    }))}
              ...
              {roomList.length === 0 && (
                <p className="text-xs text-pine/50">
                  No rooms added yet — click "+ Add room". You can add the same or different rooms
              ...
              {roomList.map((rm, idx) => (
                ...
                >
                  <span className="text-sm font-semibold flex-1">
                    {rm.room_no}
                    {rm.room_name ? ` · ${rm.room_name}` : ''}
                  </span>
                  <div className="flex flex-wrap items-center gap-2">
                    <input
                      type="date"
                      className="input !py-1 !w-36"
                      value={rm.from_date || editForm.check_in}
                      onChange={(e) =>
                        setRoomList((prev) =>
                          prev.map((r, i) => (i === idx ? { ...r, from_date: e.target.value } : r)),
                        )
                      }
                    />
                    <input
                      type="date"
                      className="input !py-1 !w-36"
                      value={rm.to_date || editForm.check_out}
                      onChange={(e) =>
                        setRoomList((prev) =>
                          prev.map((r, i) => (i === idx ? { ...r, to_date: e.target.value } : r)),
                        )
                      }
                    />
                    <input
                      ...
                      value={rm.rate}
                      onChange={(e) => updateRoomRateInModal(idx, e.target.value)}
                    />
                    <button
            ...
            <fieldset className="border border-leaf rounded-xl p-4 mb-4">
              ...
              <p className="text-xs text-pine/50 mb-3">
                Select any items included with this booking. Prices entered here are saved against
                the reservation but only posted to the bill when you choose to (Overview tab → Post
                addon charges).
              </p>
              <div className="flex flex-wrap gap-2 mb-2">
                ...
                <input
                  ...
                  min="1"
                  value={newAddon.qty}
                  onChange={(e) => setNewAddon({ ...newAddon, qty: e.target.value })}
                />
                <Button variant="ghost" size="sm" onClick={addAddonItem}>
                  <Plus size={14} />
                </Button>
              </div>
              {addonList.length === 0 && (
                <p className="text-xs text-pine/40 py-1">No items added.</p>
              )}
              {addonList.map((a, idx) => (
                <div
                  key={idx}
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-sm money text-pine/70">
                      {fmtBDT(Number(a.price) * Number(a.qty))}
                    </span>
                    <button
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {editForm.discount_type === 'percentage' ? (
                  ...
                  <input
                    ...
                    onChange={(e) => setEditForm({ ...editForm, discount_val: e.target.value })}
                  />
                )}
              </div>
            </fieldset>
            ...
            <fieldset className="border border-leaf rounded-xl p-4 mb-5">
              ...
              {editForm.terms_conditions || company?.terms_conditions ? (
                ...
                <p className="text-sm text-pine/40 italic py-3">
                  No terms configured. Go to Settings → Company to add default terms.
                </p>
              )}
              <p className="text-xs text-pine/45 mt-1.5">
                Auto-pulled from company Settings. To change, go to{' '}
                <b>Settings → Branding → Terms & Conditions</b>.
              </p>
            </fieldset>

            <div className="flex flex-wrap gap-3 justify-end border-t border-leaf pt-4">
              <Button variant="ghost" onClick={() => setQuoteEditorOpen(false)}>
                Cancel
              </Button>
