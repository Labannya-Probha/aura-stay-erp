...
function roomLabels(assignments = []) {
  return assignments
    .map((assignment) => {
      ...
      const name = room.room_name || room.room_type

      return {
        id: room.id,
        number,
        name: name || "Room",
        type: room.room_type || name || "Room",
        status: room.status || "—",
        housekeepingStatus:
...
async function loadReservations({
  ...
  if (chargesResult.error) {
    console.warn(
      "Front office folio totals unavailable:",
  ...
  if (paymentsResult.error) {
    console.warn(
      "Front office payment totals unavailable:",
...
export async function getRoomRack() {
  ...
  for (const reservation of inHouse) {
    for (const room of reservation.rooms || []) {
      guestByRoom.set(room.id, reservation)
    }
  }
