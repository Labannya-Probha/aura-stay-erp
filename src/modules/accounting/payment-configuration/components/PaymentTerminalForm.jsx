...
export default function PaymentTerminalForm({
  ...
  async function submit(e) {
    ...
    if (!values.settlement_account_id)
      next.settlement_account_id = 'Settlement account is required.'
    setErrors(next)
    ...
    await onSubmit({
      ...
      terminal_id: values.terminal_id.trim() || null,
    ...
    <form onSubmit={submit} className="space-y-6" noValidate>
      <div>
        <h2 className="text-lg font-semibold text-slate-950">
          {initialValue?.id ? 'Update terminal' : 'Create terminal'}
        </h2>
        <p className="mt-1 text-sm text-slate-500">
          Configure identification, merchant mapping and settlement routing.
        </p>
      </div>
      ...
      <fieldset disabled={isSaving} className="grid gap-4 sm:grid-cols-2 disabled:opacity-70">
        <Field label="Terminal name" error={errors.terminal_name}>
          <input
            autoFocus
            name="terminal_name"
            value={values.terminal_name}
            onChange={change}
            aria-invalid={Boolean(errors.terminal_name)}
            className={input(errors.terminal_name)}
            placeholder="Front Desk POS"
          />
        </Field>
        <Field label="Terminal code" error={errors.terminal_code}>
          <input
            name="terminal_code"
            value={values.terminal_code}
            onChange={change}
            aria-invalid={Boolean(errors.terminal_code)}
            className={input(errors.terminal_code)}
            placeholder="FD-POS-01"
          />
        </Field>
        <Field label="Payment method" error={errors.payment_method}>
          <SearchableSelect
            value={values.payment_method}
            onChange={(value) => setValues((current) => ({ ...current, payment_method: value }))}
        ...
        <Field label="Provider">
          <input
            ...
            onChange={change}
            className={input()}
            placeholder="Visa / Mastercard / bKash"
          />
        </Field>
        ...
        >
          <SearchableSelect
            ...
            onChange={(value) =>
              setValues((current) => ({ ...current, settlement_account_id: value }))
            }
            options={settlementAccounts.map((account) => ({
              value: account.id,
              label: `${account.account_code} — ${account.account_name}`,
              sublabel: account.bank_name || account.account_type || '',
            }))}
      ...
      <label className="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
        ...
        />
        <span>
          <span className="block text-sm font-medium text-slate-800">Active terminal</span>
          <span className="block text-xs text-slate-500">
            Available for operational payment collection.
          </span>
        </span>
      </label>
      ...
      <div className="sticky bottom-0 -mx-5 flex justify-end gap-3 border-t border-slate-200 bg-white px-5 py-4 sm:-mx-6 sm:px-6">
        ...
        >
          {isSaving ? 'Saving…' : initialValue?.id ? 'Update terminal' : 'Create terminal'}
        </button>
      </div>
    </form>
  )
...
function input(error = false) {
  return `min-h-10 w-full rounded-xl border bg-white px-3 py-2 text-sm outline-none focus:ring-2 ${error ? 'border-red-300 focus:ring-red-100' : 'border-slate-300 focus:ring-slate-100'}`
}
