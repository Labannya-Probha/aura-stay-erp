...
export default function Reservations({ openReservation, userName }) {
  ...
  const load = async () => {
    const { data } = await supabase.from('reservations')
      .select('id,res_no,reservation_name,status,check_in,check_out,pax_adults,pax_children,source,created_at, guests:primary_guest_id(full_name,phone)')
      ...
      <div className="card overflow-hidden">
        <table className="w-full">
          <thead><tr>
            ...
            <th className="th">Pax</th><th className="th">Source</th><th className="th">Status</th>
          </tr></thead>
          <tbody>
            {filtered.map((r) => (
            ...
            {filtered.length === 0 && <tr><td className="td text-pine/50" colSpan={6}>No reservations match. Create the first query to begin.</td></tr>}
          </tbody>
        </table>
      </div>
...
function NewReservation({ close, openReservation, userName }) {
  ...
  const [f, setF] = useState({
    ...
    check_in: t, check_out: t, pax_adults: 2, pax_children: 0, source: 'Phone', notes: '',
  })
