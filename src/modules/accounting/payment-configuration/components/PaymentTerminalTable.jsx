...
function H({ children, align = 'left' }) {
  return (
    <th
      className={`px-4 py-3 text-${align} text-xs font-semibold uppercase tracking-wider text-slate-500`}
    >
      {children}
    </th>
  )
...
function C({ children, align = 'left', mono = false }) {
  return (
    <td
      className={`px-4 py-3.5 text-${align} text-sm text-slate-700 ${mono ? 'font-mono text-xs' : ''}`}
    >
      {children}
    </td>
  )
