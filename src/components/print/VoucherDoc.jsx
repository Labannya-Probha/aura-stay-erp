...
export default function VoucherDoc({ entry, lines = [], company, voucherType }) {
  ...
  const rt = { ...cell, textAlign: 'right', fontFamily: 'Inter, sans-serif' }
    ...
    <div className="print-a4-doc" style={{ maxWidth: '186mm', margin: '0 auto', color: '#000' }}>
      ...
      >
        ...
        <div style={{ flex: 1, textAlign: company?.logo_url ? 'left' : 'center' }}>
          ...
          >
            {company?.legal_name || company?.name || 'Company'}
          </div>
      ...
      <table style={{ width: '100%', fontSize: 11, marginBottom: 8 }}>
        <tbody>
          <tr>
            <td>
              <b>Voucher No:</b> {entry.jv_no || '—'}
            </td>
            <td style={{ textAlign: 'right' }}>
              <b>Date:</b> {fmtDate(entry.jv_date)}
            </td>
          </tr>
          {type !== 'JV' && cashLine && (
            <tr>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {lines.map((l, i) => (
            <tr key={i}>
              <td style={{ ...ct, fontFamily: 'Inter, sans-serif' }}>{l.code || '—'}</td>
              <td style={cell}>
                <b>{l.name || '—'}</b>
                {l.line_note ? (
                  <div style={{ fontSize: 10, color: '#444' }}>{l.line_note}</div>
                ) : null}
              </td>
      ...
      <div style={{ fontSize: 11, marginTop: 6 }}>
        <b>In words:</b> {takaInWords(amount)}
      </div>
      {entry.narration && (
        <div style={{ fontSize: 11, marginTop: 4 }}>
          <b>Narration:</b> {entry.narration}
        </div>
      )}
      {Math.abs(totalDr - totalCr) > 0.009 && (
        <div style={{ fontSize: 10.5, marginTop: 4, color: '#b91c1c' }}>
          ⚠ Debit and Credit are not equal — this voucher is unbalanced.
        </div>
      )}
      ...
      <table
        className="print-avoid-break"
        style={{ width: '100%', marginTop: 48, fontSize: 10.5, textAlign: 'center' }}
      >
        <tbody>
          <tr>
            <td style={{ borderTop: '1px solid #000', paddingTop: 5 }}>
              Prepared by
              <br />
              <span style={{ fontSize: 9.5 }}>{entry.posted_by || ''}</span>
            </td>
            <td style={{ width: '6%' }}></td>
      ...
      <div style={{ fontSize: 8.5, color: '#777', marginTop: 14, textAlign: 'center' }}>
        Prepared under the double-entry system in accordance with IFRS. System-generated voucher —{' '}
        {company?.software_name || 'Aura Stay'}.
      </div>
    </div>
  )
}
