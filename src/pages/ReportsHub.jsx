...
function DashboardTab({ co }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      const days = Math.max(1, nightsBetween(from, to) + 1)
      const capacity = (rooms?.length || 0) * days
      setData({ revByCat, payByMethod, roomNights, adr: roomNights > 0 ? roomRev / roomNights : 0, occupancy: capacity > 0 ? (roomNights / capacity) * 100 : 0, capacity, totalRev: Object.values(revByCat).reduce((a, v) => a + v, 0), totalPay: Object.values(payByMethod).reduce((a, v) => a + v, 0), resCount: (res || []).length })
    ...
    <div className="space-y-5">
      ...
      {data && (<>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <Panel title="Revenue by category">{Object.entries(data.revByCat).sort((a,b)=>b[1]-a[1]).map(([k,v])=><Bar key={k} label={k} val={v} max={data.totalRev} />)}</Panel>
          <Panel title="Collections by method">{Object.entries(data.payByMethod).sort((a,b)=>b[1]-a[1]).map(([k,v])=><Bar key={k} label={k} val={v} max={data.totalPay} />)}</Panel>
        </div>
      </>)}
    </div>
  )
...
function SalesReportsTab({ co }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      const dueList = (dues || []).map(r => { const b = (r.folio_charges||[]).reduce((a,c)=>a+ +c.total,0); const p = (r.payments||[]).reduce((a,p)=>a+ +p.amount,0); return { res_no: r.res_no, name: r.reservation_name, due: +(b-p).toFixed(2) } }).filter(r => r.due > 0.01)
      setData({ revByCat, payByMethod, roomNights, dueList, adr: roomNights>0?roomRev/roomNights:0, occupancy: capacity>0?(roomNights/capacity)*100:0, capacity, totalRev: Object.values(revByCat).reduce((a,v)=>a+v,0), totalPay: Object.values(payByMethod).reduce((a,v)=>a+v,0) })
    } catch (e) { setErr(e.message) } finally { setLoading(false) }
  }
  useEffect(() => { run() }, [])
  const onExport = () => data && exportXLSX(`Sales_${from}_${to}.xlsx`, [
    ...
    { name: 'Outstanding', rows: [['Res No','Guest','Due ৳'],...data.dueList.map(d=>[d.res_no,d.name,d.due])] },
  ])
  const onPrint = () => {
    if (!data) return
    ...
    printToPDF('Sales Report', `${companyHeader(co,'Sales & Reservations Report',`${from} to ${to}`)}<div class="section"><b>Revenue by Category</b><table><thead><tr><th>Category</th><th class="money">৳</th></tr></thead><tbody>${revRows}</tbody></table></div><div class="section"><b>Outstanding Dues</b><table><thead><tr><th>Res No</th><th>Guest</th><th class="money">Due ৳</th></tr></thead><tbody>${dueRows||'<tr><td colspan="3">None</td></tr>'}</tbody></table></div>`)
  }
  return (
    <div className="space-y-5">
      ...
      {data && (<>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Stat label="Total revenue" val={fmtBDT(data.totalRev)} accent /><Stat label="Collections" val={fmtBDT(data.totalPay)} />
          <Stat label="Occupancy" val={`${data.occupancy.toFixed(1)}%`} sub={`${data.roomNights}/${data.capacity} room-nights`} /><Stat label="ADR" val={fmtBDT(data.adr)} />
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <Panel title="Revenue by category">{Object.entries(data.revByCat).sort((a,b)=>b[1]-a[1]).map(([k,v])=><Bar key={k} label={k} val={v} max={data.totalRev} />)}</Panel>
          <Panel title="Collections by method">{Object.entries(data.payByMethod).sort((a,b)=>b[1]-a[1]).map(([k,v])=><Bar key={k} label={k} val={v} max={data.totalPay} />)}</Panel>
        </div>
        <Tbl heads={[{label:'Res No',key:'res_no'},{label:'Guest',key:'name'},{label:'Due',key:'due',right:true,fmt:fmtBDT,red:true}]} rows={data.dueList} />
      </>)}
    </div>
  )
}
  ...
  const run = async () => {
    ...
    try {
      const [{ data: rooms }, { data: res }, { data: ch }] = await Promise.all([
        ...
        supabase.from('reservations').select('room_type,status').gte('check_in', from).lte('check_in', to),
      ...
      setData({ totalRooms, capacity, roomNights, occupancy, adr, revpar, roomRev, byType, resCount: res?.length || 0 })
    } catch (e) { setErr(e.message) } finally { setLoading(false) }
  }
  useEffect(() => { run() }, [])
    ...
    <div className="space-y-5">
      ...
      {data && (<>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <Stat label="Total rooms" val={data.totalRooms} /><Stat label="Occupancy" val={`${data.occupancy.toFixed(1)}%`} sub={`${data.roomNights}/${data.capacity}`} accent />
          <Stat label="ADR" val={fmtBDT(data.adr)} /><Stat label="RevPAR" val={fmtBDT(data.revpar)} accent /><Stat label="Room revenue" val={fmtBDT(data.roomRev)} />
        </div>
...
function GuestLedgerTab({ co }) {
  ...
  const run = async () => {
    setLoading(true); setErr(null)
    try {
      const { data: res } = await supabase.from('reservations').select('res_no,reservation_name,check_in,check_out,status,folio_charges(total),payments(amount)').in('status', ['CHECKED_IN','CHECKED_OUT','CONFIRMED']).gte('check_in', from).lte('check_in', to)
      setRows((res || []).map(r => { const c = (r.folio_charges||[]).reduce((a,x)=>a+ +x.total,0); const p = (r.payments||[]).reduce((a,x)=>a+ +x.amount,0); return { res_no: r.res_no, name: r.reservation_name, checkin: r.check_in, checkout: r.check_out, status: r.status, charges: +c.toFixed(2), paid: +p.toFixed(2), balance: +(c-p).toFixed(2) } }))
    } catch (e) { setErr(e.message) } finally { setLoading(false) }
  }
  useEffect(() => { run() }, [])
  ...
  const onExport = () => rows && exportXLSX(`GuestLedger_${from}_${to}.xlsx`, [{ name: 'Guest Ledger', rows: [['Res No','Guest','Check-in','Status','Charges ৳','Paid ৳','Balance ৳'],...rows.map(r=>[r.res_no,r.name,r.checkin,r.status,r.charges,r.paid,r.balance]),['','','','TOTAL',totals?.charges,totals?.paid,totals?.balance]] }])
  const onPrint = () => {
    if (!rows) return
    ...
    printToPDF('Guest Ledger', `${companyHeader(co,'Guest Ledger',`${from} to ${to}`)}<table><thead><tr><th>Res No</th><th>Guest</th><th>Check-in</th><th>Status</th><th class="money">Charges ৳</th><th class="money">Paid ৳</th><th class="money">Balance ৳</th></tr></thead><tbody>${trs}<tr class="total-row"><td colspan="4">TOTAL</td><td class="money">${fmtBDT(totals?.charges)}</td><td class="money">${fmtBDT(totals?.paid)}</td><td class="money">${fmtBDT(totals?.balance)}</td></tr></tbody></table>`)
  }
  return (
    <div className="space-y-5">
      <DateRange from={from} to={to} setFrom={setFrom} setTo={setTo} onRun={run} data={rows} onExport={onExport} onPrint={onPrint} />
      {err && <Err msg={err} />}{loading && <Loading />}
      {rows && (<><div className="grid grid-cols-3 gap-4"><Stat label="Total charges" val={fmtBDT(totals.charges)} /><Stat label="Total paid" val={fmtBDT(totals.paid)} /><Stat label="Outstanding balance" val={fmtBDT(totals.balance)} accent /></div>
        <Tbl heads={[{label:'Res No',key:'res_no'},{label:'Guest',key:'name'},{label:'Check-in',key:'checkin'},{label:'Status',key:'status'},{label:'Charges',key:'charges',right:true,fmt:fmtBDT},{label:'Paid',key:'paid',right:true,fmt:fmtBDT},{label:'Balance',key:'balance',right:true,fmt:fmtBDT,red:true}]} rows={rows} footRow={totals} /></>)}
    </div>
  )
}
...
function CityLedgerTab({ co }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      const { data: res } = await supabase.from('reservations').select('agency_id,folio_charges(total),payments(amount)').not('agency_id', 'is', null).gte('check_in', from).lte('check_in', to)
      const map = {}; for (const a of agencies || []) map[a.id] = { name: a.name, charges: 0, paid: 0 }
      for (const r of res || []) { if (!map[r.agency_id]) continue; map[r.agency_id].charges += (r.folio_charges||[]).reduce((a,c)=>a+ +c.total,0); map[r.agency_id].paid += (r.payments||[]).reduce((a,p)=>a+ +p.amount,0) }
      setRows(Object.values(map).map(v => ({ agency: v.name, charges: +v.charges.toFixed(2), paid: +v.paid.toFixed(2), balance: +(v.charges-v.paid).toFixed(2) })))
    } catch (e) { setErr(e.message) } finally { setLoading(false) }
  }
  useEffect(() => { run() }, [])
  const totals = rows ? { charges: rows.reduce((a,r)=>a+r.charges,0), paid: rows.reduce((a,r)=>a+r.paid,0), balance: rows.reduce((a,r)=>a+r.balance,0) } : null
  const onExport = () => rows && exportXLSX(`CityLedger_${from}_${to}.xlsx`, [{ name: 'City Ledger', rows: [['Agency','Charges ৳','Paid ৳','Balance ৳'],...rows.map(r=>[r.agency,r.charges,r.paid,r.balance]),['TOTAL',totals.charges,totals.paid,totals.balance]] }])
  const onPrint = () => { if (!rows) return; const trs = rows.map(r=>`<tr><td>${r.agency}</td><td class="money">${fmtBDT(r.charges)}</td><td class="money">${fmtBDT(r.paid)}</td><td class="money">${fmtBDT(r.balance)}</td></tr>`).join(''); printToPDF('City Ledger', `${companyHeader(co,'City Ledger — Agency Receivables',`${from} to ${to}`)}<table><thead><tr><th>Agency</th><th class="money">Charges ৳</th><th class="money">Paid ৳</th><th class="money">Balance ৳</th></tr></thead><tbody>${trs}<tr class="total-row"><td>TOTAL</td><td class="money">${fmtBDT(totals?.charges)}</td><td class="money">${fmtBDT(totals?.paid)}</td><td class="money">${fmtBDT(totals?.balance)}</td></tr></tbody></table>`) }
  return (
    <div className="space-y-5">
      <DateRange from={from} to={to} setFrom={setFrom} setTo={setTo} onRun={run} data={rows} onExport={onExport} onPrint={onPrint} />
      {err && <Err msg={err} />}{loading && <Loading />}
      {rows && (<><div className="grid grid-cols-3 gap-4"><Stat label="Total billed" val={fmtBDT(totals.charges)} /><Stat label="Total collected" val={fmtBDT(totals.paid)} /><Stat label="Outstanding" val={fmtBDT(totals.balance)} accent /></div>
...
function AgencyCommissionTab({ co }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      const { data: res } = await supabase.from('reservations').select('agency_id,folio_charges(base_amount,discount)').not('agency_id','is',null).gte('check_in',from).lte('check_in',to)
      const map = {}; for (const a of agencies||[]) map[a.id]={name:a.name,rate:+a.commission_rate,rev:0}
      for (const r of res||[]) { if (!map[r.agency_id]) continue; map[r.agency_id].rev+=(r.folio_charges||[]).reduce((a,c)=>a+ +c.base_amount- +c.discount,0) }
  ...
  const onExport = () => rows && exportXLSX(`AgencyCommission_${from}_${to}.xlsx`, [{ name: 'Agency Commission', rows: [['Agency','Rate %','Revenue ৳','Commission ৳'],...rows.map(r=>[r.agency,r.rate,r.revenue,r.commission]),['TOTAL','','',total]] }])
  const onPrint = () => { if (!rows) return; const trs=rows.map(r=>`<tr><td>${r.agency}</td><td class="money">${r.rate}%</td><td class="money">${fmtBDT(r.revenue)}</td><td class="money">${fmtBDT(r.commission)}</td></tr>`).join(''); printToPDF('Agency Commission',`${companyHeader(co,'Agency Commission Report',`${from} to ${to}`)}<table><thead><tr><th>Agency</th><th class="money">Rate %</th><th class="money">Revenue ৳</th><th class="money">Commission ৳</th></tr></thead><tbody>${trs}<tr class="total-row"><td colspan="3">TOTAL</td><td class="money">${fmtBDT(total)}</td></tr></tbody></table>`) }
  return (
    <div className="space-y-5">
      <DateRange from={from} to={to} setFrom={setFrom} setTo={setTo} onRun={run} data={rows} onExport={onExport} onPrint={onPrint} />
      {err && <Err msg={err} />}{loading && <Loading />}
      {rows && (<><Stat label="Total commission due" val={fmtBDT(total)} accent /><Tbl heads={[{label:'Agency',key:'agency'},{label:'Rate',key:'rate',right:true,fmt:v=>`${v}%`},{label:'Revenue',key:'revenue',right:true,fmt:fmtBDT},{label:'Commission',key:'commission',right:true,fmt:fmtBDT}]} rows={rows} /></>)}
    </div>
  )
}
...
function ShareholderTab({ co }) {
  ...
  const onExport = () => rows && exportXLSX(`Shareholder_${from}_${to}.xlsx`,[{name:'Shareholder Entitlement',rows:[['Shareholder','Rate %','Revenue ৳','Commission ৳','Free Stay Bal'],...rows.map(r=>[r.name,r.rate,r.revenue,r.commission,r.freeStay]),['TOTAL','','',totComm,'']]}])
  const onPrint = () => { if (!rows) return; const trs=rows.map(r=>`<tr><td>${r.name}</td><td class="money">${r.rate}%</td><td class="money">${fmtBDT(r.revenue)}</td><td class="money">${fmtBDT(r.commission)}</td><td class="money">${r.freeStay}</td></tr>`).join(''); printToPDF('Shareholder',`${companyHeader(co,'Shareholder Entitlement Report',`${from} to ${to}`)}<table><thead><tr><th>Shareholder</th><th class="money">Rate %</th><th class="money">Revenue ৳</th><th class="money">Commission ৳</th><th class="money">Free Stay</th></tr></thead><tbody>${trs}<tr class="total-row"><td colspan="3">TOTAL</td><td class="money">${fmtBDT(totComm)}</td><td></td></tr></tbody></table>`) }
  return (
    <div className="space-y-5">
      <DateRange from={from} to={to} setFrom={setFrom} setTo={setTo} onRun={run} data={rows} onExport={onExport} onPrint={onPrint} />
      {err && <Err msg={err} />}{loading && <Loading />}
      {rows && (<><Stat label="Total commission entitlement" val={fmtBDT(totComm)} accent /><Tbl heads={[{label:'Shareholder',key:'name'},{label:'Rate',key:'rate',right:true,fmt:v=>`${v}%`},{label:'Revenue',key:'revenue',right:true,fmt:fmtBDT},{label:'Commission',key:'commission',right:true,fmt:fmtBDT},{label:'Free Stay Bal',key:'freeStay',right:true}]} rows={rows} /></>)}
    </div>
  )
}
...
function PosReportsTab({ co }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      const totalSales=(orders||[]).reduce((a,o)=>a+ +o.total,0); const settled=(orders||[]).filter(o=>o.status==='SETTLED').reduce((a,o)=>a+ +o.total,0); const room=(orders||[]).filter(o=>o.status==='CHARGED_TO_ROOM').reduce((a,o)=>a+ +o.total,0)
      const byItem={}; for (const it of items||[]) { if (!byItem[it.menu_item_name]) byItem[it.menu_item_name]={qty:0,rev:0}; byItem[it.menu_item_name].qty+= +it.qty; byItem[it.menu_item_name].rev+= +it.qty* +it.unit_price }
      setData({totalSales,settled,room,orderCount:(orders||[]).length,byItem})
    } catch (e) { setErr(e.message) } finally { setLoading(false) }
  }
  useEffect(() => { run() }, [])
  const onExport = () => data && exportXLSX(`POS_${from}_${to}.xlsx`,[{name:'Summary',rows:[['Total Sales',data.totalSales],['Settled',data.settled],['Room',data.room],['Orders',data.orderCount]]},{name:'Items Sold',rows:[['Item','Qty','Revenue ৳'],...Object.entries(data.byItem).sort((a,b)=>b[1].rev-a[1].rev).map(([k,v])=>[k,v.qty,v.rev])]}])
  const onPrint = () => { if (!data) return; const trs=Object.entries(data.byItem).sort((a,b)=>b[1].rev-a[1].rev).map(([k,v])=>`<tr><td>${k}</td><td class="money">${v.qty}</td><td class="money">${fmtBDT(v.rev)}</td></tr>`).join(''); printToPDF('POS Report',`${companyHeader(co,'Restaurant POS Sales Summary',`${from} to ${to}`)}<div class="kpi-grid"><div class="kpi"><div class="kpi-label">Total Sales</div><div class="kpi-val">${fmtBDT(data.totalSales)}</div></div><div class="kpi"><div class="kpi-label">Settled</div><div class="kpi-val">${fmtBDT(data.settled)}</div></div><div class="kpi"><div class="kpi-label">Charged to Room</div><div class="kpi-val">${fmtBDT(data.room)}</div></div><div class="kpi"><div class="kpi-label">Orders</div><div class="kpi-val">${data.orderCount}</div></div></div><table><thead><tr><th>Item</th><th class="money">Qty</th><th class="money">Revenue ৳</th></tr></thead><tbody>${trs}</tbody></table>`) }
  return (
    <div className="space-y-5">
      <DateRange from={from} to={to} setFrom={setFrom} setTo={setTo} onRun={run} data={data} onExport={onExport} onPrint={onPrint} />
      {err && <Err msg={err} />}{loading && <Loading />}
      {data && (<><div className="grid grid-cols-2 md:grid-cols-4 gap-4"><Stat label="Total POS sales" val={fmtBDT(data.totalSales)} accent /><Stat label="Settled" val={fmtBDT(data.settled)} /><Stat label="Charged to room" val={fmtBDT(data.room)} /><Stat label="Orders" val={data.orderCount} /></div>
        <Tbl heads={[{label:'Item',key:'item'},{label:'Qty',key:'qty',right:true},{label:'Revenue',key:'rev',right:true,fmt:fmtBDT}]} rows={Object.entries(data.byItem).sort((a,b)=>b[1].rev-a[1].rev).map(([k,v])=>({item:k,qty:v.qty,rev:v.rev}))} /></>)}
    </div>
  )
}
...
function KOTTab({ co }) {
  ...
  const run = async () => {
    setLoading(true); setErr(null)
    try {
      const { data } = await supabase.from('pos_orders').select('order_no,created_at,status,total,table_no,waiter_name').gte('created_at',`${from}T00:00:00Z`).lte('created_at',`${to}T23:59:59Z`).order('created_at',{ascending:false})
      setRows(data || [])
    } catch (e) { setErr(e.message) } finally { setLoading(false) }
  }
  useEffect(() => { run() }, [])
  const total = rows ? rows.reduce((a,r)=>a+(r.status!=='CANCELLED'? +r.total:0),0) : 0
  const onExport = () => rows && exportXLSX(`KOT_${from}_${to}.xlsx`,[{name:'KOT Register',rows:[['Order No','Date/Time','Table','Status','Amount ৳'],...rows.map(r=>[r.order_no,new Date(r.created_at).toLocaleString('en-BD'),r.table_no||'—',r.status,r.status!=='CANCELLED'? +r.total:0])]}])
  const onPrint = () => { if (!rows) return; const trs=rows.map(r=>`<tr><td>${r.order_no}</td><td>${new Date(r.created_at).toLocaleString('en-BD')}</td><td>${r.table_no||'—'}</td><td>${r.status}</td><td class="money">${r.status!=='CANCELLED'?fmtBDT(r.total):'—'}</td></tr>`).join(''); printToPDF('KOT Register',`${companyHeader(co,'KOT Register',`${from} to ${to}`)}<table><thead><tr><th>Order No</th><th>Date/Time</th><th>Table</th><th>Status</th><th class="money">Amount ৳</th></tr></thead><tbody>${trs}<tr class="total-row"><td colspan="4">TOTAL</td><td class="money">${fmtBDT(total)}</td></tr></tbody></table>`) }
  return (
    <div className="space-y-5">
      <DateRange from={from} to={to} setFrom={setFrom} setTo={setTo} onRun={run} data={rows} onExport={onExport} onPrint={onPrint} />
      {err && <Err msg={err} />}{loading && <Loading />}
      {rows && (<><Stat label="Total F&B revenue (non-cancelled)" val={fmtBDT(total)} accent /><Tbl heads={[{label:'Order No',key:'order_no'},{label:'Date/Time',key:'created_at',fmt:v=>new Date(v).toLocaleString('en-BD')},{label:'Table',key:'table_no'},{label:'Status',key:'status'},{label:'Amount',key:'total',right:true,fmt:(v,r)=>r.status==='CANCELLED'?'—':fmtBDT(v)}]} rows={rows} /></>)}
    </div>
  )
}
...
function FnBRevenueTab({ co }) {
  ...
  const run = async () => {
    setLoading(true); setErr(null)
    try {
      const { data } = await supabase.from('pos_orders').select('created_at,total,status').neq('status','CANCELLED').gte('created_at',`${from}T00:00:00Z`).lte('created_at',`${to}T23:59:59Z`)
      const byDay={}; for (const o of data||[]) { const day=o.created_at.slice(0,10); byDay[day]=(byDay[day]||0)+ +o.total }
  ...
  const onExport = () => rows && exportXLSX(`FnB_Daily_${from}_${to}.xlsx`,[{name:'F&B Daily Revenue',rows:[['Date','Revenue ৳'],...rows.map(r=>[r.date,r.rev]),['TOTAL',total]]}])
  const onPrint = () => { if (!rows) return; const trs=rows.map(r=>`<tr><td>${r.date}</td><td class="money">${fmtBDT(r.rev)}</td></tr>`).join(''); printToPDF('F&B Revenue',`${companyHeader(co,'F&B Daily Revenue Report',`${from} to ${to}`)}<table><thead><tr><th>Date</th><th class="money">Revenue ৳</th></tr></thead><tbody>${trs}<tr class="total-row"><td>TOTAL</td><td class="money">${fmtBDT(total)}</td></tr></tbody></table>`) }
  return (
    <div className="space-y-5">
      <DateRange from={from} to={to} setFrom={setFrom} setTo={setTo} onRun={run} data={rows} onExport={onExport} onPrint={onPrint} />
      {err && <Err msg={err} />}{loading && <Loading />}
      {rows && (<><Stat label="Total F&B revenue" val={fmtBDT(total)} accent /><Tbl heads={[{label:'Date',key:'date'},{label:'Revenue',key:'rev',right:true,fmt:fmtBDT}]} rows={rows} footRow={{rev:total}} /></>)}
    </div>
  )
}
...
function PLTab({ co }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      let income=0,cogs=0,opex=0,interest=0,tax=0; const incomeLines=[],cogsLines=[],opexLines=[]
      ...
      setData({income,cogs,grossProfit,opex,ebit,interest,ebt,tax,netProfit,incomeLines,cogsLines,opexLines})
    } catch (e) { setErr(e.message) } finally { setLoading(false) }
  }
  useEffect(() => { run() }, [])
  ...
  return (
    <div className="space-y-5">
      ...
      {err && <Err msg={err} />}{loading && <Loading />}
      {data && (<>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4"><Stat label="Revenue" val={fmtBDT(data.income)} /><Stat label="Gross profit" val={fmtBDT(data.grossProfit)} /><Stat label="EBIT" val={fmtBDT(data.ebit)} /><Stat label="Net profit / (loss)" val={fmtBDT(data.netProfit)} accent /></div>
        <div className="card p-5 space-y-1 font-mono text-sm">
          {[{label:'REVENUE',val:data.income,bold:true},...data.incomeLines.map(l=>({label:l.name,val:l.val,indent:true})),{label:'COST OF GOODS SOLD',val:null,bold:true,sep:true},...data.cogsLines.map(l=>({label:l.name,val:l.val,indent:true})),{label:'Gross Profit',val:data.grossProfit,bold:true,sep:true},{label:'OPERATING EXPENSES',val:null,bold:true,sep:true},...data.opexLines.map(l=>({label:l.name,val:l.val,indent:true})),{label:'Operating Profit (EBIT)',val:data.ebit,bold:true,sep:true},{label:'Finance Cost',val:-data.interest,indent:true},{label:'Profit before Tax',val:data.ebt,bold:true},{label:'Income Tax',val:-data.tax,indent:true},{label:'NET PROFIT / (LOSS)',val:data.netProfit,bold:true,sep:true}].map((row,i)=>(
            <div key={i} className={`flex justify-between py-0.5 ${row.sep?'border-t border-leaf mt-1 pt-1':''} ${row.bold?'font-bold':''}`}>
              ...
              {row.val!==null&&row.val!==undefined&&<span className={row.val<0?'text-red-600':''}>{fmtBDT(row.val)}</span>}
            </div>
          ))}
        </div>
      </>)}
...
function BalanceSheetTab({ co }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      for (const a of accounts) { const b=balMap[a.id]||{debit:0,credit:0}; const net=a.normal_side==='DEBIT'?b.debit-b.credit:b.credit-b.debit; if (net===0||!sections[a.type]) continue; sections[a.type].push({code:a.code,name:a.name,val:+net.toFixed(2)}) }
  ...
  const onPrint = () => {
    ...
    printToPDF('Balance Sheet',`${companyHeader(co,'Statement of Financial Position (IAS 1)',`As at ${asOf}`)}<table><thead><tr><th>Description</th><th class="money">৳</th></tr></thead><tbody>${sec(data.ASSET,'ASSETS',data.totAsset)}${sec(data.LIABILITY,'LIABILITIES',data.totLiab)}${sec(data.EQUITY,'EQUITY',data.totEq)}<tr class="total-row"><td><b>TOTAL LIABILITIES + EQUITY</b></td><td class="money"><b>${fmtBDT(data.totLiab+data.totEq)}</b></td></tr></tbody></table>`)
  }
  return (
    <div className="space-y-5">
      ...
      {err && <Err msg={err} />}{loading && <Loading />}
      {data && (<>
        <div className="grid grid-cols-3 gap-4"><Stat label="Total assets" val={fmtBDT(data.totAsset)} /><Stat label="Total liabilities" val={fmtBDT(data.totLiab)} /><Stat label="Total equity" val={fmtBDT(data.totEq)} accent /></div>
        {['ASSET','LIABILITY','EQUITY'].map(type=>(
          <div key={type} className="card p-4">
            ...
            {data[type].map((r,i)=>(<div key={i} className="flex justify-between py-1 border-b border-leaf/50 text-sm"><span className="pl-2 text-pine/80">{r.name}</span><span className="money">{fmtBDT(r.val)}</span></div>))}
            <div className="flex justify-between py-2 font-bold text-sm mt-1"><span>Total</span><span className="money">{fmtBDT(type==='ASSET'?data.totAsset:type==='LIABILITY'?data.totLiab:data.totEq)}</span></div>
          </div>
        ))}
      </>)}
    </div>
...
function CashFlowTab({ co }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      const receipts=(pm||[]).reduce((a,p)=>a+ +p.amount,0); const roomBilled=(ch||[]).filter(c=>c.charge_type==='ROOM').reduce((a,c)=>a+ +c.total,0); const fnbBilled=(ch||[]).filter(c=>c.charge_type==='RESTAURANT').reduce((a,c)=>a+ +c.total,0)
  ...
  return (
    <div className="space-y-5">
      <p className="text-xs text-pine/50">IAS 7 — Indirect method.</p>
      <DateRange from={from} to={to} setFrom={setFrom} setTo={setTo} onRun={run} data={data} onExport={onExport} onPrint={onPrint} />
      {err && <Err msg={err} />}{loading && <Loading />}
      {data && (<>
        <Stat label="Net cash from operating activities" val={fmtBDT(data.netOps)} accent />
        <div className="card p-5 space-y-1 font-mono text-sm">
          {[{label:'A. OPERATING ACTIVITIES',group:true},{label:'Cash received from guests',val:data.receipts,indent:true},{label:'Room revenue billed',val:data.roomBilled,indent:true},{label:'F&B revenue billed',val:data.fnbBilled,indent:true},{label:'Net cash from operations',val:data.netOps,bold:true,sep:true},{label:'B. INVESTING ACTIVITIES',group:true},{label:'Capital expenditure',val:0,indent:true},{label:'C. FINANCING ACTIVITIES',group:true},{label:'Loan repayments / proceeds',val:0,indent:true},{label:'NET CASH INCREASE / (DECREASE)',val:data.netOps,bold:true,sep:true}].map((row,i)=>(
...
function TrialBalanceTab({ co }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <DateRange from={from} to={to} setFrom={setFrom} setTo={setTo} onRun={run} data={rows} onExport={onExport} onPrint={onPrint} />
      {err && <Err msg={err} />}{loading && <Loading />}
      {rows && (<><div className="grid grid-cols-2 gap-4"><Stat label="Total debits" val={fmtBDT(totDr)} /><Stat label="Total credits" val={fmtBDT(totCr)} /></div>
...
function LedgerTab({ co }) {
  ...
  const onExport = () => rows && exportXLSX(`Ledger_${accName}_${from}_${to}.xlsx`,[{name:'General Ledger',rows:[['JV No','Date','Narration','Debit ৳','Credit ৳','Balance ৳'],...rows.map(r=>[r.jv_no,r.date,r.narration,r.debit,r.credit,r.balance])]}])
  ...
  return (
    <div className="space-y-5">
      ...
      {err && <Err msg={err} />}{loading && <Loading />}
      {rows && <Tbl heads={[{label:'JV No',key:'jv_no'},{label:'Date',key:'date'},{label:'Narration',key:'narration'},{label:'Debit',key:'debit',right:true,fmt:fmtBDT},{label:'Credit',key:'credit',right:true,fmt:fmtBDT},{label:'Balance',key:'balance',right:true,fmt:fmtBDT}]} rows={rows} />}
    </div>
  )
}
...
function BankBookTab({ co }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      let running=0; setRows((data||[]).map(p=>{running+= +p.amount; return {date:p.received_date,narration:p.reservations?.reservation_name||p.reference_no||'—',method:p.method,debit:+p.amount,balance:+running.toFixed(2)}}))
    } catch (e) { setErr(e.message) } finally { setLoading(false) }
  }
  useEffect(() => { run() }, [])
  const total=rows?rows.reduce((a,r)=>a+r.debit,0):0
  const onExport = () => rows && exportXLSX(`BankBook_${from}_${to}.xlsx`,[{name:'Bank Book',rows:[['Date','Narration','Method','Debit ৳','Balance ৳'],...rows.map(r=>[r.date,r.narration,r.method,r.debit,r.balance]),['','','TOTAL',total,'']]}])
  const onPrint = () => { if (!rows) return; const trs=rows.map(r=>`<tr><td>${r.date}</td><td>${r.narration}</td><td>${r.method}</td><td class="money">${fmtBDT(r.debit)}</td><td class="money">${fmtBDT(r.balance)}</td></tr>`).join(''); printToPDF('Bank Book',`${companyHeader(co,'Bank Book',`${from} to ${to}`)}<table><thead><tr><th>Date</th><th>Narration</th><th>Method</th><th class="money">Debit ৳</th><th class="money">Balance ৳</th></tr></thead><tbody>${trs}<tr class="total-row"><td colspan="3">TOTAL</td><td class="money">${fmtBDT(total)}</td><td></td></tr></tbody></table>`) }
  return (
    <div className="space-y-5">
      <p className="text-xs text-pine/50">Bank, bKash, Nagad, and card transactions.</p>
      <DateRange from={from} to={to} setFrom={setFrom} setTo={setTo} onRun={run} data={rows} onExport={onExport} onPrint={onPrint} />
      {err && <Err msg={err} />}{loading && <Loading />}
      {rows && (<><Stat label="Total bank receipts" val={fmtBDT(total)} accent /><Tbl heads={[{label:'Date',key:'date'},{label:'Narration',key:'narration'},{label:'Method',key:'method'},{label:'Debit',key:'debit',right:true,fmt:fmtBDT},{label:'Balance',key:'balance',right:true,fmt:fmtBDT}]} rows={rows} /></>)}
    </div>
  )
}
...
function CashBookTab({ co }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      let running=0; setRows((data||[]).map(p=>{running+= +p.amount; return {date:p.received_date,narration:p.reservations?.reservation_name||p.reference_no||'Cash receipt',debit:+p.amount,balance:+running.toFixed(2)}}))
    } catch (e) { setErr(e.message) } finally { setLoading(false) }
  }
  useEffect(() => { run() }, [])
  const total=rows?rows.reduce((a,r)=>a+r.debit,0):0
  const onExport = () => rows && exportXLSX(`CashBook_${from}_${to}.xlsx`,[{name:'Cash Book',rows:[['Date','Narration','Cash In ৳','Balance ৳'],...rows.map(r=>[r.date,r.narration,r.debit,r.balance]),['','TOTAL',total,'']]}])
  const onPrint = () => { if (!rows) return; const trs=rows.map(r=>`<tr><td>${r.date}</td><td>${r.narration}</td><td class="money">${fmtBDT(r.debit)}</td><td class="money">${fmtBDT(r.balance)}</td></tr>`).join(''); printToPDF('Cash Book',`${companyHeader(co,'Cash Book',`${from} to ${to}`)}<table><thead><tr><th>Date</th><th>Narration</th><th class="money">Cash In ৳</th><th class="money">Balance ৳</th></tr></thead><tbody>${trs}<tr class="total-row"><td colspan="2">TOTAL</td><td class="money">${fmtBDT(total)}</td><td></td></tr></tbody></table>`) }
  return (
    <div className="space-y-5">
      <p className="text-xs text-pine/50">Cash receipts only.</p>
      <DateRange from={from} to={to} setFrom={setFrom} setTo={setTo} onRun={run} data={rows} onExport={onExport} onPrint={onPrint} />
      {err && <Err msg={err} />}{loading && <Loading />}
      {rows && (<><Stat label="Total cash received" val={fmtBDT(total)} accent /><Tbl heads={[{label:'Date',key:'date'},{label:'Narration',key:'narration'},{label:'Cash In',key:'debit',right:true,fmt:fmtBDT},{label:'Balance',key:'balance',right:true,fmt:fmtBDT}]} rows={rows} /></>)}
    </div>
  )
}
...
function BankReconTab({ co }) {
  ...
  const onPrint = () => { if (!rows) return; printToPDF('Bank Reconciliation',`${companyHeader(co,'Bank Reconciliation Statement',`As at ${asOf}`)}<table><thead><tr><th>Description</th><th class="money">৳</th></tr></thead><tbody><tr><td>Book Balance (system)</td><td class="money">${fmtBDT(rows.bookBal)}</td></tr><tr><td>Bank Statement Balance</td><td class="money">${fmtBDT(rows.bankBal)}</td></tr><tr class="total-row"><td>Difference (unreconciled)</td><td class="money">${fmtBDT(rows.diff)}</td></tr></tbody></table>`) }
  return (
    <div className="space-y-5">
...
function RetainedEarningsTab({ co }) {
  ...
  const onPrint = () => { if (!data) return; printToPDF('Retained Earnings',`${companyHeader(co,'Statement of Retained Earnings (IAS 1)',`Year ${year}`)}<table><thead><tr><th>Description</th><th class="money">৳</th></tr></thead><tbody><tr><td>Opening retained earnings (1 Jan ${year})</td><td class="money">${fmtBDT(data.opening)}</td></tr><tr><td>Add: Net profit for the year</td><td class="money">${fmtBDT(data.netProfit)}</td></tr><tr><td>Less: Dividends declared</td><td class="money">(${fmtBDT(data.div)})</td></tr><tr class="total-row"><td><b>Closing retained earnings (31 Dec ${year})</b></td><td class="money"><b>${fmtBDT(data.closing)}</b></td></tr></tbody></table>`) }
  return (
    <div className="space-y-5">
      ...
      {data&&(<div className="card p-5 font-mono text-sm space-y-2">
        {[{label:`Opening RE (1 Jan ${year})`,val:data.opening},{label:'Add: Net profit',val:data.netProfit},{label:'Less: Dividends',val:-data.div},{label:`Closing RE (31 Dec ${year})`,val:data.closing,bold:true,sep:true}].map((row,i)=>(
          <div key={i} className={`flex justify-between py-1 ${row.sep?'border-t border-leaf font-bold':''}`}><span>{row.label}</span><span className={row.val<0?'text-red-600':''}>{fmtBDT(row.val)}</span></div>
        ))}
      </div>)}
    </div>
  )
...
function NAVTab({ co }) {
  ...
  const onPrint = () => { if (!data) return; printToPDF('NAV Report',`${companyHeader(co,'Net Asset Value (NAV) Report',`As at ${asOf}`)}<table><thead><tr><th>Description</th><th class="money">৳</th></tr></thead><tbody><tr><td>Total Assets</td><td class="money">${fmtBDT(data.totAsset)}</td></tr><tr><td>Less: Total Liabilities</td><td class="money">(${fmtBDT(data.totLiab)})</td></tr><tr class="total-row"><td><b>NAV</b></td><td class="money"><b>${fmtBDT(data.nav)}</b></td></tr><tr><td>Number of Shares</td><td class="money">${data.shareCount.toLocaleString()}</td></tr><tr class="total-row"><td><b>NAV per Share</b></td><td class="money"><b>${fmtBDT(data.navPerShare)}</b></td></tr></tbody></table>`) }
  return (
    <div className="space-y-5">
      <p className="text-xs text-pine/50">Net Asset Value = Total Assets − Total Liabilities.</p>
      <div className="flex items-end gap-2 flex-wrap">
        <div><label className="label">As at date</label><input type="date" className="input !w-40" value={asOf} onChange={e=>setAsOf(e.target.value)} /></div>
        <div><label className="label">No. of shares</label><input type="number" className="input money !w-40" value={shares} onChange={e=>setShares(e.target.value)} placeholder="e.g. 1000" /></div>
        <button className="btn-primary" onClick={run}>Run</button>
        {data&&<button className="btn-ghost" onClick={onPrint}><Printer size={15}/> PDF</button>}
      </div>
      {err&&<Err msg={err}/>}{loading&&<Loading/>}
      {data&&(<div className="grid grid-cols-2 md:grid-cols-4 gap-4"><Stat label="Total assets" val={fmtBDT(data.totAsset)} /><Stat label="Total liabilities" val={fmtBDT(data.totLiab)} /><Stat label="NAV" val={fmtBDT(data.nav)} accent /><Stat label="NAV per share" val={fmtBDT(data.navPerShare)} /></div>)}
    </div>
  )
}
...
function APAgingTab({ co }) {
  ...
  const onExport = () => rows && exportXLSX(`APAging_${asOf}.xlsx`,[{name:'AP Aging',rows:[['Vendor','0–30','31–60','61–90','90+','Total'],...rows.map(r=>[r.vendor,r.current,r.d30,r.d60,r.d90,r.total]),['TOTAL',tot?.current,tot?.d30,tot?.d60,tot?.d90,tot?.total]]}])
  const onPrint = () => { if (!rows) return; const trs=rows.map(r=>`<tr><td>${r.vendor}</td><td class="money">${fmtBDT(r.current)}</td><td class="money">${fmtBDT(r.d30)}</td><td class="money">${fmtBDT(r.d60)}</td><td class="money">${fmtBDT(r.d90)}</td><td class="money">${fmtBDT(r.total)}</td></tr>`).join(''); printToPDF('AP Aging',`${companyHeader(co,'Accounts Payable Aging Report',`As at ${asOf}`)}<table><thead><tr><th>Vendor</th><th class="money">0–30</th><th class="money">31–60</th><th class="money">61–90</th><th class="money">90+</th><th class="money">Total ৳</th></tr></thead><tbody>${trs}<tr class="total-row"><td>TOTAL</td><td class="money">${fmtBDT(tot?.current)}</td><td class="money">${fmtBDT(tot?.d30)}</td><td class="money">${fmtBDT(tot?.d60)}</td><td class="money">${fmtBDT(tot?.d90)}</td><td class="money">${fmtBDT(tot?.total)}</td></tr></tbody></table>`) }
  return (
    <div className="space-y-5">
...
function ARAgingTab({ co }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      const today=new Date(asOf)
      const mapped=(res||[]).map(r=>{const b=(r.folio_charges||[]).reduce((a,c)=>a+ +c.total,0);const p=(r.payments||[]).reduce((a,p)=>a+ +p.amount,0);const bal=b-p;if(bal<0.01)return null;const days=Math.floor((today-new Date(r.check_out))/86400000);return{guest:r.reservation_name,checkOut:r.check_out,days,balance:+bal.toFixed(2)}}).filter(Boolean)
  ...
  const onExport = () => rows && exportXLSX(`ARAging_${asOf}.xlsx`,[{name:'AR Aging',rows:[['Guest','Check-out','Days','Balance ৳'],...rows.lines.map(r=>[r.guest,r.checkOut,r.days,r.balance]),['TOTAL','','',totBal]]}])
  const onPrint = () => { if (!rows) return; const trs=rows.lines.map(r=>`<tr><td>${r.guest}</td><td>${r.checkOut}</td><td class="money">${r.days}</td><td class="money">${fmtBDT(r.balance)}</td></tr>`).join(''); printToPDF('AR Aging',`${companyHeader(co,'Accounts Receivable Aging Report',`As at ${asOf}`)}<div class="kpi-grid"><div class="kpi"><div class="kpi-label">0–30 days</div><div class="kpi-val">${fmtBDT(rows.current)}</div></div><div class="kpi"><div class="kpi-label">31–60 days</div><div class="kpi-val">${fmtBDT(rows.d30)}</div></div><div class="kpi"><div class="kpi-label">61–90 days</div><div class="kpi-val">${fmtBDT(rows.d60)}</div></div><div class="kpi"><div class="kpi-label">90+ days</div><div class="kpi-val">${fmtBDT(rows.d90)}</div></div></div><table><thead><tr><th>Guest</th><th>Check-out</th><th class="money">Days</th><th class="money">Balance ৳</th></tr></thead><tbody>${trs}<tr class="total-row"><td colspan="3">TOTAL</td><td class="money">${fmtBDT(totBal)}</td></tr></tbody></table>`) }
  return (
    <div className="space-y-5">
      <div className="flex items-end gap-2 flex-wrap"><div><label className="label">As at date</label><input type="date" className="input !w-40" value={asOf} onChange={e=>setAsOf(e.target.value)} /></div><button className="btn-primary" onClick={run}>Run</button>{rows&&<><button className="btn-ghost" onClick={onExport}><FileDown size={15}/> Excel</button><button className="btn-ghost" onClick={onPrint}><Printer size={15}/> PDF</button></>}</div>
      {err&&<Err msg={err}/>}{loading&&<Loading/>}
      {rows&&(<><div className="grid grid-cols-2 md:grid-cols-4 gap-4"><Stat label="0–30 days" val={fmtBDT(rows.current)} /><Stat label="31–60 days" val={fmtBDT(rows.d30)} /><Stat label="61–90 days" val={fmtBDT(rows.d60)} /><Stat label="90+ days" val={fmtBDT(rows.d90)} accent /></div>
...
export default function ReportsHub({ userName, role }) {
  ...
  useEffect(() => {
    ...
    if (t && TABS.find(x => x.id === t)) {
      setActiveTab(t)
      setActiveGroup(TABS.find(x => x.id === t).group)
    }
  }, [location.search])
    ...
    <div className="space-y-5">
          ...
          return (
            <button
              ...
              className={`px-4 py-2 text-sm font-semibold rounded-t-lg transition-colors ${
                isActive
                  ? `bg-white border border-leaf border-b-white -mb-px ${st.active}`
                  : `${st.inactive} hover:text-pine`
              }`}
            >
              {group}
            </button>
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                isActive
                  ...
                  : 'text-pine/60 hover:bg-leaf/40 hover:text-pine'
              }`}
            >
              <Icon size={12} />
              {t.label}
      ...
      {activeTabDef && (
        <div className="flex items-center gap-2">
          ...
          <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${GROUP_BADGE[activeTabDef.group]}`}>
            {activeTabDef.group}
          </span>
        </div>
      )}
