...
export default function EmployeeProfile({ employee, company, userName, back }) {
  ...
  return (
    <div className="space-y-4">
      <button className="btn-ghost !py-1" onClick={back}>
        <ChevronLeft size={15} /> Back to employee list
      </button>
      ...
      <div className="card p-5">
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-xs text-pine/40 mt-1">
              Joined {employee.join_date ? fmtDate(employee.join_date) : '—'} ·{' '}
              {employee.phone || 'No phone on file'}
            </p>
          </div>
          <span
            className={`status-chip ${employee.status === 'ACTIVE' ? 'bg-forest/15 text-forest' : 'bg-stone-200 text-stone-700'}`}
          >
            {employee.status}
          </span>
        </div>
        {summary && (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mt-4 pt-4 border-t border-leaf">
            ...
            <Stat
              ...
              value={summary.latest_net_payable ? fmtBDT(summary.latest_net_payable) : '—'}
            />
            <Stat
              label="Compliance"
              value={`${summary.compliance_total_count - summary.compliance_pending_count} / ${summary.compliance_total_count}`}
              highlight={summary.compliance_pending_count > 0}
            />
          </div>
        )}
      </div>
      ...
      <div className="tab-strip-responsive border-b border-leaf">
        {TABS.map((t) => (
          <button
            ...
            className={`tab-button-responsive px-4 py-2 text-sm font-semibold rounded-t-lg ${tab === t.key ? 'bg-white border border-leaf border-b-white text-forest -mb-px' : 'text-pine/60 hover:text-pine'}`}
          >
            {t.label}
          </button>
        ))}
...
function Stat({ label, value, highlight }) {
  return (
    <div>
      ...
      <div className={`text-lg font-display font-bold ${highlight ? 'text-amber' : 'text-pine'}`}>
        {value}
      </div>
    </div>
  )
...
function OverviewPanel({ employee }) {
  return (
    <div className="card p-5 grid grid-cols-2 gap-4 text-sm">
      <div>
        <span className="text-pine/50">Full Name</span>
        <div className="font-medium">{employee.full_name}</div>
      </div>
      <div>
        <span className="text-pine/50">Employee Code</span>
        <div className="font-medium">{employee.emp_code}</div>
      </div>
      <div>
        <span className="text-pine/50">Designation</span>
        <div className="font-medium">{employee.designation || '—'}</div>
      </div>
      <div>
        <span className="text-pine/50">Department</span>
        <div className="font-medium">{employee.department || '—'}</div>
      </div>
      <div>
        <span className="text-pine/50">Join Date</span>
        <div className="font-medium">{employee.join_date ? fmtDate(employee.join_date) : '—'}</div>
      </div>
      <div>
        <span className="text-pine/50">Phone</span>
        <div className="font-medium">{employee.phone || '—'}</div>
      </div>
      <div>
        <span className="text-pine/50">NID</span>
        <div className="font-medium">{employee.nid || '—'}</div>
      </div>
      <div>
        <span className="text-pine/50">Gross Salary</span>
        <div className="font-medium">{fmtBDT(employee.gross_salary)}</div>
      </div>
      <div className="col-span-2">
        <span className="text-pine/50">Address</span>
        <div className="font-medium">{employee.address || '—'}</div>
      </div>
      <div className="col-span-2">
        <span className="text-pine/50">Notes</span>
        <div className="font-medium">{employee.notes || '—'}</div>
      </div>
    </div>
  )
}
    ...
    <div className="space-y-3">
      <div className="card p-3 flex items-end gap-3 flex-wrap">
        ...
        <div className="flex gap-2 text-xs ml-auto">
          {['P', 'A', 'L', 'H', 'OFF'].map((s) => (
            <span key={s} className="status-chip bg-leaf/50 text-pine/70">
              {s}: {counts[s] || 0}
            </span>
          ))}
        </div>
            ...
            {!loading &&
              rows.map((r) => (
                <tr key={r.id}>
                  ...
                  <td className="td">
                    <span
                      className={`status-chip ${r.status === 'A' ? 'bg-red-100 text-red-600' : r.status === 'P' ? 'bg-forest/15 text-forest' : 'bg-stone-200 text-stone-700'}`}
                    >
                      {r.status}
                    </span>
                  </td>
...
function LeavePanel({ employeeId }) {
  ...
  useEffect(() => {
    Promise.all([
      supabase
        ...
        .select('*, leave_types(name, annual_days)')
        .eq('employee_id', employeeId)
  ...
  return (
    <div className="space-y-4">
      <div className="card overflow-hidden">
        <div className="px-4 py-2 text-xs font-semibold text-pine/50 bg-leaf/30">
          Leave Applications
        </div>
        <table className="w-full">
          <thead>
            ...
            {!loading &&
              rows.map((r) => (
                <tr key={r.id}>
                  ...
                  <td className="td">
                    <span
                      className={`status-chip ${r.status === 'APPROVED' ? 'bg-forest/15 text-forest' : r.status === 'REJECTED' ? 'bg-red-100 text-red-600' : 'bg-amber/20 text-amber'}`}
                    >
                      {r.status}
                    </span>
                  </td>
          ...
          <tbody>
            {compRows.map((r) => (
              <tr key={r.id}>
                ...
                <td className="td">
                  <span
                    className={`status-chip ${r.used ? 'bg-stone-200 text-stone-700' : 'bg-forest/15 text-forest'}`}
                  >
            ...
            {compRows.length === 0 && (
              <tr>
                <td className="td text-pine/40" colSpan={4}>
                  No compensatory leave recorded.
                </td>
              </tr>
            )}
          </tbody>
...
function PayslipsPanel({ employee, company }) {
  ...
  useEffect(() => {
    supabase
      .from('v_employee_payslip_history')
      .select('*')
      .eq('employee_id', employee.id)
      .then(({ data }) => {
            ...
            {!loading &&
              rows.map((r) => (
                <tr key={r.payslip_id}>
                  ...
                  <td className="td">
                    <span
                      className={`status-chip ${r.run_status === 'PAID' ? 'bg-forest/15 text-forest' : r.run_status === 'APPROVED' ? 'bg-amber/20 text-amber' : 'bg-stone-200 text-stone-700'}`}
                    >
                      {r.run_status}
                    </span>
                  </td>
                  ...
                  <td className="td money text-right">
                    {fmtBDT(+r.absent_deduction + +r.advance_deduction + +r.other_deduction)}
                  </td>
                  <td className="td money text-right font-bold text-forest">
                    {fmtBDT(r.net_payable)}
                  </td>
                  <td className="td text-right">
                    <button className="btn-ghost !py-1" onClick={() => setPrintSlip(r)}>
                      <Printer size={13} />
                    </button>
                  </td>
                </tr>
    ...
    <div style={{ maxWidth: 600, margin: '0 auto', color: '#000' }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          ...
          <tr>
            <td style={cell}>Internet/Telephone Allowance</td>
...
function CompliancePanel({ employeeId }) {
  ...
  const load = () => {
    supabase
      .from('v_employee_compliance_register')
      .select('*')
      .eq('employee_id', employeeId)
      .order('phase')
      .order('form_code')
  ...
  const toggle = async (r) => {
    if (r.status === 'COMPLETED') {
      await supabase
        ...
        .eq('employee_id', employeeId)
        .eq('compliance_item_id', r.compliance_item_id)
    } else {
      ...
      if (existing) {
        await supabase
          .from('employee_compliance_records')
          .update({ status: 'COMPLETED', record_date: todayISO() })
          .eq('id', existing.id)
      } else {
        await supabase
          .from('employee_compliance_records')
      ...
      <table className="w-full">
        ...
        <tbody>
          ...
          {!loading &&
            rows.map((r) => (
              <tr key={r.compliance_item_id}>
                ...
                <td className="td">
                  <span
                    className={`status-chip ${r.status === 'COMPLETED' ? 'bg-forest/15 text-forest' : 'bg-stone-200 text-stone-600'}`}
                  >
                    {r.status}
                  </span>
                </td>
                <td className="td">
                  ...
                  >
                    {r.status === 'COMPLETED' ? (
                      <>
                        <CheckCircle2 size={12} /> Mark Pending
                      </>
                    ) : (
                      <>
          ...
          {!loading && rows.length === 0 && (
            <tr>
              <td className="td text-pine/40" colSpan={5}>
                No employee-level compliance items configured.
              </td>
            </tr>
          )}
        </tbody>
