...
export function PosReceipt({ order, items, company, mushakNo }) {
  const row = { padding: '2px 0', fontSize: 11 }
  const num = { textAlign: 'right', fontFamily: '"IBM Plex Mono", monospace' }
  return (
    <div style={{ maxWidth: 320, margin: '0 auto', color: '#000' }}>
      ...
      <table style={{ width: '100%', borderTop: '1px dashed #000', borderBottom: '1px dashed #000', borderCollapse: 'collapse' }}>
        <tbody>
          {items.map((it, i) => (
            <tr key={i}>
              <td style={row}>{it.item_name} <span style={{ fontFamily: '"IBM Plex Mono", monospace' }}>× {Number(it.qty)}</span></td>
      ...
      <div style={{ textAlign: 'center', fontSize: 10, marginTop: 8, fontWeight: 700 }}>
      ...
      {mushakNo && <div style={{ textAlign: 'center', fontSize: 9, marginTop: 2 }}>Mushak-6.3 issued: {mushakNo}</div>}
      {order.status === 'CHARGED_TO_ROOM' && <div style={{ textAlign: 'center', fontSize: 8.5, marginTop: 2 }}>Tax invoice (Mushak-6.3) will be issued with the final guest bill at check-out.</div>}
      <div style={{ textAlign: 'center', fontSize: 9, marginTop: 8, fontStyle: 'italic', color: '#2E7D32' }}>{company?.invoice_footer}</div>
    </div>
  )
}
...
export function KitchenTicket({ order, items }) {
  return (
    <div style={{ maxWidth: 320, margin: '0 auto', color: '#000', fontFamily: '"IBM Plex Mono", monospace' }}>
      <div style={{ textAlign: 'center', borderBottom: '2px solid #000', paddingBottom: 4 }}>
        <div style={{ fontSize: 15, fontWeight: 700, letterSpacing: 2 }}>KITCHEN ORDER</div>
        <div style={{ fontSize: 11 }}>{order.order_no} · {timeOf(order.created_at)}</div>
      ...
      <table style={{ width: '100%', marginTop: 6 }}>
        <tbody>
          {items.map((it, i) => (
            <tr key={i}>
              <td style={{ fontSize: 15, fontWeight: 700, padding: '3px 0', width: 40 }}>{Number(it.qty)} ×</td>
              <td style={{ fontSize: 14, padding: '3px 0' }}>{it.item_name}</td>
            </tr>
          ))}
        </tbody>
      </table>
      {order.notes && <div style={{ marginTop: 6, fontSize: 12, borderTop: '1px dashed #000', paddingTop: 4 }}><b>NOTE:</b> {order.notes}</div>}
    </div>
  )
}
