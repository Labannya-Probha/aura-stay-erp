...
function resolveReportContext(reportPayload = {}) {
  ...
  return {
    companyName: safeText(
      summary.company_name || audit.companyName || filters.companyName || filters.company || 'Aura Stay ERP',
    ),
    propertyName: safeText(
      summary.property_name || audit.propertyName || filters.property || filters.propertyName || 'All Properties',
    ),
...
function formatTabularCellValue(value, column) {
  ...
  if (type.includes('currency') || type.includes('amount') || type.includes('debit') || type.includes('credit')) {
...
function createFinancialStatementPdf(reportPayload) {
  ...
  function drawHeader() {
    ...
    document.text(safeText(report?.name || report?.title || 'Financial Statement'), pageWidth / 2, cursorY, {
      align: 'center',
    })
  ...
  for (const row of sortedRows) {
    ...
    document.text(safeText(row.label || row.account_name || row.line_item), marginLeft + indent, cursorY, {
...
function createTabularReportPdf(reportPayload) {
  ...
  function drawFooter() {
    ...
    document.setFontSize(7)
    document.setTextColor(90)
    document.text(`Generated by ${context.generatedBy} at ${context.generatedAt}`, marginLeft, pageHeight - 10)
    document.text(
      `Prepared: ${context.preparedBy || '-'}  Reviewed: ${context.reviewedBy || '-'}  Approved: ${context.approvedBy || '-'}  Printed: ${context.printedBy || '-'}`,
      marginLeft,
      pageHeight - 6,
