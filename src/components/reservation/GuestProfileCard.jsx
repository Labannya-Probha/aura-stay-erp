...
import { summarizeGuestHistory } from '../../lib/guestHistory'
...
export function GuestProfileCard({ guest, reservationId, isAdmin, userName, reload, flash }) {
  ...
  const loadProfile = async () => {
    ...
    const [{ data: profileData }, { data: reservationsData }] = await Promise.all([
      withTenantScope(
        supabase.from('v_guest_profile').select('*').eq('id', guest.id),
      ).maybeSingle(),
      withTenantScope(
        supabase
          .from('reservations')
          .select('id,res_no,check_in,check_out,status')
          .eq('primary_guest_id', guest.id)
          .order('check_in', { ascending: false }),
      ),
    ])

    const reservationIds = (reservationsData || [])
    ...
    const derivedHistory = summarizeGuestHistory(reservationsData || [], chargesData)
    ...
    setForm({
      ...
      anniversary_date: fallbackProfile.anniversary_date || '',
      preferences: fallbackProfile.preferences || [],
      notes: fallbackProfile.notes || '',
    })
  }
  ...
  const adjustPoints = async () => {
    if (!ptForm.change || isNaN(Number(ptForm.change))) {
      flash('Enter a valid points amount.')
      return
    }
    setBusy(true)
    ...
    if (newBalance < 0) {
      flash('Points cannot go below 0.')
      setBusy(false)
      return
    }
    ...
    <div className="card p-5 lg:col-span-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="bg-leaf/20 rounded-xl p-4">
          <div className="text-xs font-bold text-pine/50 uppercase tracking-wide mb-3">
            Stay History
          </div>
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold">
                {p?.last_stay_date ? fmtDate(p.last_stay_date) : '—'}
              </span>
            </div>
            {p?.active_stays > 0 && (
              <div className="flex justify-between">
                <span className="text-pine/60">Currently in</span>
                <span className="status-chip bg-forest/15 text-forest text-xs">
                  {p.active_stays} active
                </span>
              </div>
            )}
          </div>
          ...
          {editing ? (
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold">
                  {p?.anniversary_date ? fmtDate(p.anniversary_date) : '—'}
                </span>
              </div>
            </div>
          )}
          ...
          {showLedger && (
            <div className="mt-3 space-y-1 max-h-40 overflow-y-auto">
              ...
              {ledger.map((l) => (
                <div
                  key={l.id}
                  className="flex justify-between text-xs py-1 border-b border-leaf/40"
                >
                  <span className="text-pine/60">
                    {l.reason} · {l.created_at?.slice(0, 10)}
                  </span>
                  <span
                    className={
                      l.change > 0 ? 'text-forest font-semibold' : 'text-red-500 font-semibold'
                    }
                  >
                    {l.change > 0 ? '+' : ''}
                    {l.change} → {l.balance_after}
                  </span>
                </div>
              ))}
            </div>
        ...
        <div className="lg:col-span-3">
          ...
          <div className="flex flex-wrap gap-2 mb-2 min-h-[32px]">
            {(editing ? form.preferences : p?.preferences || []).map((pref) => (
              <span
                key={pref}
                className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${editing ? 'bg-forest/15 text-forest' : 'bg-leaf text-pine'}`}
              >
                {pref}
                {editing && (
                  <button
                    onClick={() => removePreference(pref)}
                    className="text-forest/60 hover:text-red-500 ml-0.5"
                  >
                    ×
                  </button>
                )}
