...
function preferTenantColor(value, paletteValue, seededDefault) {
  ...
  if (safeColor(paletteValue, null) && normalized === seededDefault)
    return safeColor(paletteValue, null)
  return normalized
}
...
export function buildTenantTheme(company = {}, logoPalette = null) {
  ...
  const primary =
    ...
    preferTenantColor(
      safeCompany.brand_primary,
      palette.primary,
      SEEDED_BRAND_DEFAULTS.brandPrimary,
    ) ||
  ...
  const secondary =
    preferTenantColor(
      safeCompany.secondary_color,
      palette.secondary || mix(primary, '#FFFFFF', 0.22),
      SEEDED_BRAND_DEFAULTS.secondary,
    ) ||
  ...
  const sidebarBg =
    preferTenantColor(
      safeCompany.sidebar_bg_color,
      darken(primary, 0.34),
      SEEDED_BRAND_DEFAULTS.sidebarBg,
    ) ||
    preferTenantColor(
      safeCompany.brand_primary,
      darken(primary, 0.34),
  ...
  const reportHeader =
    preferTenantColor(
      safeCompany.report_header_color,
      primaryDark,
      SEEDED_BRAND_DEFAULTS.reportHeader,
    ) ||
  ...
  const tableHeader =
    preferTenantColor(
      safeCompany.table_header_color,
      mix(primary, '#FFFFFF', 0.9),
      SEEDED_BRAND_DEFAULTS.secondary,
    ) || secondary
  ...
  return {
    ...AEDS_DEFAULT_THEME,
    name:
      safeCompany.software_name ||
      safeCompany.name ||
      safeCompany.tenant_name ||
      AEDS_DEFAULT_THEME.name,
    primary,
    primaryDark,
...
export function themeToCssVars(theme = AEDS_DEFAULT_THEME) {
  ...
  const darkRgb = hexToRgb(theme.sidebarBg || theme.primaryDark || darken(theme.primary, 0.34)) || {
    r: 27,
    g: 77,
    b: 46,
  }
  ...
  return {
    ...
    '--aeds-font-mono': theme.fontMono || AEDS_DEFAULT_THEME.fontMono,
    ...
    '--sidebar-text':
      theme.sidebarText || getReadableText(theme.sidebarBg || theme.primaryDark || theme.primary),
