  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const { data: r } = await withTenantScope(
        supabase.from('reservations').select('*, agencies(*), shareholders(*)').eq('id', id),
      )
        .limit(1)
        .maybeSingle()
      setRes(r)
      if (r?.primary_guest_id) {
        const { data: g } = await withTenantScope(
          supabase.from('guests').select('*').eq('id', r.primary_guest_id),
        ).single()
        setGuest(g)
      } else {
        setGuest(null)
      ...
      if (r?.company_id) {
        const { data: gc } = await withTenantScope(
          supabase.from('companies').select('*').eq('id', r.company_id),
        ).maybeSingle()
        setGuestCompany(gc)
      } else {
        setGuestCompany(null)
      ...
      const [
        ...
        withTenantScope(
          supabase.from('reservation_guests').select('*').eq('reservation_id', id),
        ...
        withTenantScope(supabase.from('folio_charges').select('*').eq('reservation_id', id)).order(
          'charge_date',
        ),
        withTenantScope(supabase.from('payments').select('*').eq('reservation_id', id)).order(
          'received_date',
        ),
        withTenantScope(supabase.from('invoices').select('*').eq('reservation_id', id)).order(
          'created_at',
          { ascending: false },
        ),
        withTenantScope(
          supabase.from('reservation_addons').select('*').eq('reservation_id', id),
        ).order('created_at'),
        withTenantScope(supabase.from('tax_config').select('*')),
        getCompanySettingsQuery('*').limit(1).maybeSingle(),
        withTenantScope(supabase.from('guest_ids').select('*').eq('reservation_id', id)).order(
          'created_at',
        ),
      ])
      setResGuests(rg || [])
