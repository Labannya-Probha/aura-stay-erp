    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-4 gap-3">
        ...
        <div className="sm:text-right">
          ...
          <div className="font-display text-2xl font-bold money text-forest">
            {fmtBDT(advancePaid)}
          </div>
        </div>
      </div>
          ...
          <div className="mt-5 rounded-2xl border border-leaf bg-white/70 p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
              <div>
                ...
                <div className="font-semibold money">{Number(res.commission_pct) || 0}%</div>
              </div>
              <div>
                <span className="label">Vat/VDS</span>
                <div className="font-semibold money">{Number(res.vat_vds_pct) || 0}%</div>
              </div>
              <div>
                <span className="label">Tax/TDS</span>
                <div className="font-semibold money">{Number(res.tax_tds_pct) || 0}%</div>
              </div>
            </div>
          </div>
        )}
...
/*  PAYMENTS TAB                                                       */
/* ------------------------------------------------------------------ */
function ReservationPaymentsTab({
  res,
  guest,
  payments,
  reload,
  ...
  const saveEdit = async () => {
    ...
    if (!editForm.amount || +editForm.amount <= 0) {
      showPopover('Enter a valid amount.', 'error')
      return
    }
    ...
    const { error } = await supabase
      ...
      .eq('id', editRow.id)
    if (error) {
      showPopover(error.message || 'Update failed.', 'error')
      return
    }
    setEditRow(null)
    await reload()
  ...
  const sendNow = async () => {
    ...
    if (!sendBox.to?.trim()) {
      showPopover('Recipient is required.', 'error')
      return
    }
    ...
    if (sendBox.channel === 'WHATSAPP' && !sendBox.file) {
      ...
      await logAudit({
        ...
        entity: 'payment',
        entity_id:
          parsePaymentReference(sendBox.payment?.reference).paymentNo || sendBox.payment?.id,
        details: {
          channel: 'WHATSAPP',
          to: sendBox.to,
          reservation_id: res?.id,
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div className="sm:col-span-2">
            ...
            <SearchableSelect
              ...
              options={[
                {
                  value: guest?.full_name || 'Guest',
                  label: `👤 ${guest?.full_name || 'Guest'} (Guest)`,
                },
                ...(res.agencies
                  ? [{ value: res.agencies.name, label: `🤝 ${res.agencies.name} (Agency)` }]
                  : []),
                ...(res.shareholders
                  ? [
                      {
                        value: res.shareholders.name,
                        label: `👥 ${res.shareholders.name} (Shareholder)`,
                      },
                    ]
                  : []),
              ].filter(Boolean)}
      ...
      <div className="card overflow-hidden relative">
        {actionPopover.open && (
          <div
            className={`absolute right-4 top-3 z-10 px-3 py-2 rounded-lg shadow-lg text-xs font-semibold ${actionPopover.kind === 'error' ? 'bg-red-600 text-white' : 'bg-forest text-white'}`}
          >
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {payments.map((pm) => {
                ...
                return (
                  <tr key={pm.id}>
                    ...
                    <td className="td text-xs font-mono text-pine/80">
                      {parsePaymentReference(pm.reference).paymentNo || '—'}
                    </td>
                    ...
                    <td className="td text-xs">
                      {parsePaymentReference(pm.reference).reference || '—'}
                    </td>
                    ...
                    <td className="td text-xs">
                      {lg ? (
                        <span
                          className={`status-chip ${lg.status === 'SUCCESS' ? 'bg-forest/15 text-forest' : 'bg-red-100 text-red-600'}`}
                        >
                          {lg.status}
                        </span>
                      ) : (
              ...
              {payments.length === 0 && (
                <tr>
                  <td className="td text-pine/50" colSpan={9}>
            ...
            <tbody>
              {deliveryLogs.map((l) => (
                <tr key={l.id}>
                  ...
                  <td className="td text-xs">
                    <span
                      className={`status-chip ${l.status === 'SUCCESS' ? 'bg-forest/15 text-forest' : 'bg-red-100 text-red-600'}`}
                    >
                      {l.status}
                    </span>
                  </td>
                  <td className="td text-xs text-pine/70">
                    {l.error_message || l.provider_message || 'Delivered'}
                  </td>
                </tr>
              ))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <SearchableSelect
                  ...
                  options={['ADVANCE', 'SETTLEMENT', 'PARTIAL']}
                />
              </div>
            </div>
      ...
      {sendBox.open && (
        <div className="fixed inset-0 z-50 bg-ink/60 flex items-center justify-center p-4">
          <div className="card w-full max-w-2xl p-5 space-y-3">
            <h4 className="font-display font-semibold text-pine">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  placeholder={sendBox.channel === 'WHATSAPP' ? 'Phone number' : 'Email address'}
                />
              </div>
