  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-pine/60">Double-entry journals, trial balance, chart of accounts and fixed-asset depreciation.</p>
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="card overflow-hidden">
        <table className="w-full">
          <thead><tr><th className="th">JV No</th><th className="th">Date</th><th className="th">Narration</th><th className="th">Source</th><th className="th text-right">Amount</th></tr></thead>
          <tbody>
            {rows.map((r) => { const amt = (r.journal_lines || []).reduce((a, l) => a + +l.debit, 0); return (
              <tr key={r.id}><td className="td money font-semibold">{r.jv_no}</td><td className="td money text-xs">{fmtDate(r.jv_date)}</td><td className="td text-sm">{r.narration}</td><td className="td text-xs">{r.source}</td><td className="td money text-right">{fmtBDT(amt)}</td></tr>) })}
...
function CoaTab({ accounts, reload, flash, isAdmin }) {
  ...
  const del = async (id) => { const { error } = await supabase.from('chart_of_accounts').delete().eq('id', id); if (error) flash('Admin access required.'); else reload() }
  return (
    <div className="space-y-4">
