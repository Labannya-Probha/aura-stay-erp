  ...
  return (
    <div className="space-y-4">
      <div className="card p-4 grid grid-cols-6 gap-2">
        ...
        <Button variant="default" className="justify-center" onClick={add}><Plus size={15} /> Register</Button>
      </div>

      <div className="card overflow-hidden">
        <table className="w-full">
          <thead><tr><th className="th">Docket No</th><th className="th">Date</th><th className="th">Type</th><th className="th">Subject</th><th className="th">Party</th><th className="th"></th></tr></thead>
          <tbody>
            {rows.map((r) => (
              <tr key={r.id}>
                ...
                <td className="td">
                  <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setViewing(r)} aria-label={`View ${r.doc_no}`}>
                    <Eye size={14} />
                  </Button>
                </td>
              </tr>
            ))}
            {rows.length === 0 && <tr><td className="td text-pine/40" colSpan={6}>No documents registered.</td></tr>}
          </tbody>
        </table>
      </div>
  ...
  return (
    <Tabs value={view || ''} onValueChange={setView} className="space-y-4">
      <TabsList className="flex-wrap">
        <TabsTrigger value=""><FileText size={11} /> All</TabsTrigger>
        {DOC_TYPES.map((t) => (
          <TabsTrigger key={t} value={t}><FileText size={11} /> {t}</TabsTrigger>
        ))}
      </TabsList>
