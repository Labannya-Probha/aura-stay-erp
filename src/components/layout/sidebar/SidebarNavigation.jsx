...
} from '../../../modules/front-office/frontOffice.config'
...
function normalizeReportDepartment(report, category) {
  return (
    ...
    slugify(
      report.department || report.categoryName || category?.name || report.category || 'reports',
    )
  )
}
...
function normalizeReportSlug(report) {
  return (
    ...
    report.reportSlug ||
    report.report_slug ||
    cleanReportSlug(report.name || report.reportName || report.title || report.code)
  )
}
...
function isAccountingRoute(pathname) {
  return (
    pathname.startsWith('/accounting') || pathname === PATHS.VAT || pathname === PATHS.VAT_RETURN
  )
}
...
function getModuleVisibilityDecision(
  item,
  { role, isAdmin, privileges, modulesEnabled, visibleReservationTabs },
) {
  if (!isModuleEnabled(item.id, modulesEnabled, role)) {
    return { visible: false, reason: 'module-disabled' }
  }
  ...
  if (item.id === 'nightaudit') {
    ...
    return { visible, reason: visible ? 'allowed' : 'permission-denied-nighaudit' }
  }

  if (item.id === 'reservations') {
    const visible = visibleReservationTabs.length > 0
    return { visible, reason: visible ? 'allowed' : 'no-visible-reservation-tabs' }
  }

  if (item.id === 'master-data') {
    const visible = role === 'SUPERUSER' && can(role, 'cms', privileges)
    return { visible, reason: visible ? 'allowed' : 'superuser-or-cms-required' }
  }

  if (item.id === 'settings') {
    const visible = isAdmin || role === 'SUPERUSER' || can(role, 'settings', privileges)
    return { visible, reason: visible ? 'allowed' : 'settings-permission-required' }
  }

  const visible = can(role, item.id, privileges)
  return { visible, reason: visible ? 'allowed' : 'permission-denied' }
}
...
function ChildButton({ child, onNavigate }) {
  ...
  return (
    <Button
      ...
      className={`flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2.5 text-left text-xs font-medium transition-all focus-visible:ring-2 focus-visible:ring-white/18 focus-visible:ring-offset-0 ${
        child.active
          ? 'bg-white/10 text-white shadow-[0_6px_14px_rgba(0,0,0,0.1)]'
          : child.path
    ...
    <div>
      ...
      >
        <span className="flex min-w-0 flex-1 items-center gap-2">
          {group.icon && (
            <group.icon size={12} className="shrink-0 opacity-70" aria-hidden="true" />
          )}
      ...
      {open && (
        <div className="ml-3.5 mt-1.5 space-y-1.5 border-l border-white/8 pl-3">
...
function buildSettingsChildren({ role, isAdmin, location }) {
  return getVisibleSettingsSections({ role, isAdmin }).map((section) => ({
    ...
    active:
      getTopSegment(location.pathname) === 'settings' &&
...
function buildFrontOfficeChildren({ role, isAdmin, privileges, location }) {
  ...
  return FRONT_OFFICE_PAGES.filter(
    (page) =>
      isAdmin || role === 'SUPERUSER' || can(role, page.permission || 'frontoffice', privileges),
  )
    ...
    .map((page) => ({
      ...
      active:
        (location.pathname === PATHS.FRONT_OFFICE ||
          location.pathname.startsWith(`${PATHS.FRONT_OFFICE}/`)) &&
...
function buildAccountingChildren({ role, isAdmin, location }) {
  return SIDEBAR_ACCOUNTING_TABS.filter(
    (tab) => !tab.adminOnly || isAdmin || role === 'SUPERUSER',
  ).map((tab) => ({
    ...tab,
    active:
      tab.id === 'vat'
...
function buildHrChildren(location) {
  ...
  const groups = [
    { id: 'employee', label: 'Employee', tabIds: ['employee-entry', 'service-book', 'nominee'] },
    {
      id: 'attendance',
      label: 'Attendance',
      tabIds: ['attendance-register', 'employee-register', 'service-book-reg'],
    },
    { id: 'leave', label: 'Leave', tabIds: ['leave-entry', 'comp-leave', 'festival-leave'] },
    {
      id: 'payroll',
      label: 'Payroll',
      tabIds: ['payroll-config', 'payroll-gen', 'payroll-register'],
    },
    {
      id: 'letters',
      label: 'Letters',
...
export default function SidebarNavigation({
  ...
  const uiDebug = isUiDebugEnabled()
    ...
    () =>
      NAV_GROUPS.flatMap((group) =>
        group.items.map((item) => ({
          item,
          decision: getModuleVisibilityDecision(item, context),
        })),
      ),
    [context],
  )
  ...
  useEffect(() => {
    ...
    const hiddenModules = moduleVisibility
      .filter(({ decision }) => !decision.visible)
      .map(({ item }) => item.id)

    recordSidebarSnapshot({ visibleModules, hiddenModules })

    moduleVisibility.forEach(({ item, decision }) => {
      ...
      <nav aria-label="Primary navigation" className="flex h-full flex-col gap-4 px-3 py-3">
        ...
        {NAV_GROUPS.map((group, groupIndex) => {
          const preparedItems = group.items
            .filter((item) => getModuleVisibilityDecision(item, context).visible)
            .map((item) => ({
              item,
              nested: EXPANDABLE_MODULES.has(item.id) ? buildNestedChildren(item.id, context) : [],
            }))
            ...
            >
              ...
              <div className="space-y-1.5">
                {preparedItems.map(({ item, nested }) => {
                  ...
                  const open = searchForcesOpen || openSystemMenu === item.id
                  ...
                  return (
                    <div key={item.id} className="space-y-1.5">
                      ...
                      {open && (
                        <div className="ml-3.5 space-y-1.5 border-l border-white/8 pl-3">
                          {nested.map((child) =>
                            child.children ? (
